"""Adapter for videomae: the as-is frozen-backbone linear probe (no pretraining).

    python -m adapter --config <resolved.json> --out <dir>

VideoMAE (Tong, Song, Wang & Wang, NeurIPS 2022; arXiv:2203.12602) is ported
**eval-only**: a single `linear_eval` stage freezes the pretrained video encoder
and fits a linear probe on its temporally-averaged patch tokens. VideoMAE's masked
video autoencoding pretraining (Kinetics/SSv2, many GPU-days) is the excluded step,
so the port reuses the released checkpoint, exactly as the capture's ImageNet
linear eval does -- feeding a still image replicated `num_frames` times along the
temporal axis (never a video dataset). The probed representation is a genuine SSL
feature, so the number is comparable.

There is **no author submodule**: neither timm nor transformers carries a VideoMAE
model class here, so the model is a small self-contained Conv3d-patch-embed ViT in
the method's own `evaluate_linear_videomae.py`, and the weights are a sha256-pinned
download recorded as `backbone_artifact` in provenance.json and fetched by
`bin/fetch-weights.py`. The checkpoint is the authors' official public
`MCG-NJU/videomae-base` (a `VideoMAEForPreTraining` safetensors); its `videomae.*`
encoder tensors load into the ViT directly and the `decoder.*` pretext-decoder keys
are dropped. This stage trains nothing and produces no `encoder.pt` -- it records
...
Licence: the official `MCG-NJU/videomae-base` weights are distributed under
CC-BY-NC-4.0 (non-commercial), like this repository's other non-commercial ports.
Nothing under it is copied into this repository; the model is built from
architecture keys in the method's own evaluate script.
"""
...
# ── linear_eval config ───────────────────────────────────────────────────────
# The architecture keys let the hermetic smoke build a random tiny VideoMAE ViT;
# the shipped config pins MCG-NJU/videomae-base and names the downloaded ckpt.
# `num_frames`/`tubelet_size` are VideoMAE-specific: the still image is replicated
# `num_frames` times and the Conv3d tubelet embed consumes `tubelet_size` frames
# per temporal token.
...
def run_linear_eval(config: dict, out: Path, _run=None) -> dict:
    ...
    if _run is None:
        ...
        from evaluate_linear_videomae import run as _run
