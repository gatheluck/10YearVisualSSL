"""Linear evaluation for Rotation: a probe on the frozen AlexNet-BN encoder.

The representation is the model this port trains -- the AlexNet-BN encoder
(`get_encoder()`), which maps an image to a 4096-d feature. Images are resized to
the encoder's training input size. The probe follows the lab's ARSSL protocol:
...
The capture's research eval could select an earlier conv layer for the probe; the
port fixes the representation at the trained encoder's output so the number is a
single, comparable linear probe, as in every other self-contained port here.
"""
...
from train_step1_rotation import make_deterministic, resolve_device   # noqa: E402
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Rotation linear eval")
...
def run(args, config: "dict | None" = None, model=None) -> dict:
    ...
    if model is None:
        ...
        from train_step1_rotation import model_kwargs
        model = build_alexnet_rotation_model(**model_kwargs(train))
    encoder = model.get_encoder().to(device)
    encoder.eval()
    for p in encoder.parameters():
        p.requires_grad = False
    print(f"Rotation linear eval  device={device}  "
          f"image_size={train['image_size']}")
