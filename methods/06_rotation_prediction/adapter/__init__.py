"""Adapter for 06_rotation_prediction, step 1 and linear evaluation (Gidaris et
al., ICLR 2018).

    python -m adapter --config <resolved.json> --out <dir>

Rotation solves a pretext of predicting which of {0, 90, 180, 270} degrees was
applied to an image (step 1), then a linear probe on the AlexNet-BN encoder
(linear_eval). A self-contained re-implementation (the lab's own code) -- no
submodule. The capture's step 2 (ViT) is excluded, as in every port.

`encoder.pt` is the AlexNet-BN encoder (`encoder.*`); the 4-class rotation head
is pretext machinery and is left out. `linear_eval` reads this `encoder.pt`; the
...
# Step-2 unified ViT-B/16 (`arch: vit`), faithful to the capture's
# train_step2_vit.py. Selected by the optional `arch` key; absent/`alexnet` is
# the native path, unchanged. The key sets are disjoint on purpose (AdamW betas
# vs SGD momentum) so a knob from one path cannot sit unused in the other.
ARCHS = ("alexnet", "vit")
VIT_MODEL_KEYS = frozenset({"num_classes", "image_size", "patch_size",
...
# The AlexNet-BN encoder. The rotation head is excluded.
...
def _vit_model_kwargs(train: dict) -> dict:
    return {k: (float(train[k]) if k in _VIT_FLOATS else int(train[k]))
            for k in VIT_MODEL_KEYS}
...
def load_encoder(state_dict: dict, config: dict):
    ...
    train = config["train"]
    if train.get("arch", "alexnet") == "vit":
        from models.vit_rotation import build_vit_rotation_model
        model = build_vit_rotation_model(**_vit_model_kwargs(train))
    else:
        from models import build_alexnet_rotation_model
        from train_pretrain_rotation import model_kwargs
        model = build_alexnet_rotation_model(**model_kwargs(train))
...
def run_training(config: dict, out: Path, _run=None) -> dict:
    arch = config.get("train", {}).get("arch", "alexnet")
    if _run is None:
        if str(METHOD_DIR) not in sys.path:
            sys.path.insert(0, str(METHOD_DIR))
        if arch == "vit":
            from train_pretrain_vit_rotation import run as _run
        else:
            from train_pretrain_rotation import run as _run
...
def run_linear_eval(config: dict, out: Path, _run=None) -> dict:
    ...
    if _run is None:
        if str(METHOD_DIR) not in sys.path:
            sys.path.insert(0, str(METHOD_DIR))
        from evaluate_linear_rotation import run as _run
...
def body(ctx: adapterlib.Context) -> None:
    ...
    # ViT Step-2: also hand out the frozen backbone at each milestone so the
    # ImageNet probe can be run at epochs 100/200/300 (the CSV's Step-2 sweep).
...
def _absent_reason(config_path) -> "str | None":
    if _stage_of(config_path) != "linear_eval":
        return None
    return ("this stage fits a linear probe on the frozen AlexNet-BN encoder "
