"""Adapter for aimv2: the as-is frozen-backbone linear probe (no pretraining).

    python -m adapter --config <resolved.json> --out <dir>

AIMv2 (Fini et al., 2024) is ported **eval-only**: a single `linear_eval` stage
freezes the official multimodal-autoregressively pretrained backbone
(`aimv2_large_patch14_224.apple_pt`) and fits a linear probe on its pooled
features. AIMv2's from-scratch pretraining (billions of image-text pairs, many
GPU-days) is the excluded step, so the port reuses the released checkpoint, exactly
as the capture's as-is comparison (ARSSL Track A) does. The probed representation
is a genuine learned feature, so the number is comparable.
...
random tiny AIMv2-style ViT, so nothing is downloaded.

Licence: the AIMv2 weights are redistributed by timm under the Apple Sample Code
License (research-only). Nothing under it is copied into this repository; the model
constructor is imported from the pinned timm dependency.
"""
...
# ── linear_eval config ───────────────────────────────────────────────────────
# The architecture keys let the hermetic smoke build a random tiny AIMv2-style
# ViT; the shipped config pins the official AIMv2-Large/14 and names the
# downloaded ckpt.
...
def run_linear_eval(config: dict, out: Path, _run=None) -> dict:
    ...
    if _run is None:
        ...
        from evaluate_linear_aimv2 import run as _run
