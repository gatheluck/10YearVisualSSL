"""Linear evaluation for BYOL: a probe on the frozen online ResNet-50 backbone.

The representation is the model this port trains -- the online backbone read
through `encode()`, one 2048-d feature per image. Images use the deterministic val
pipeline (resize + centre crop, ImageNet normalisation, no augmentation -- the
same normalisation BYOL trains with). The probe follows the lab's ARSSL protocol:
features are extracted once and cached, mean-centred and L2-normalised, and a
single linear layer is trained with SGD (momentum) under a cosine schedule; top-1
and top-5 are reported.
"""
...
from train_pretrain_byol import make_deterministic, resolve_device   # noqa: E402
from data import get_linear_eval_transform                        # noqa: E402
...
def _build_loader(data_root: str, split: str, image_size: int, batch_size: int,
    ...
    dataset = datasets.ImageFolder(
        ...
        transform=get_linear_eval_transform(image_size, mode="val"))
...
def extract_features(model, loader, device):
    ...
    for imgs, lbs in loader:
        feats.append(model.encode(imgs.to(device, non_blocking=True))
                     .float().cpu())
        labels.append(lbs)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="BYOL linear eval")
...
def run(args, config: "dict | None" = None, model=None) -> dict:
    ...
    isz = int(train["image_size"])
    print(f"BYOL linear eval  device={device}  arch={train['arch']}  "
          f"img_size={isz}")
