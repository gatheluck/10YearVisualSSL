"""Feature-extraction provider for 19_byol.
...
wrapper that reuses this method's own pieces, so the knowledge of how BYOL
turns an image into a vector stays in one place:

- the frozen encoder is rebuilt from `encoder.pt` by the adapter's
  `load_encoder` (the online ResNet-50 backbone, `online_encoder.*`); the model
  is handed to `extract_features` *directly* (not through a `get_encoder()`),
  which reads it through `model.encode()` -- the 2048-d backbone feature, exactly
  as this method's own `evaluate_linear_byol.run` does;
- images go through the method's own deterministic eval pipeline
  ...
  BYOL trains on ImageNet-normalised inputs);
...
def extract_val_features(*, encoder_path: str, data_root: str, split: str,
    ...
    ev = importlib.import_module("evaluate_linear_byol")
