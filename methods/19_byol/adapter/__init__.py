"""Adapter for 19_byol, step 1 and linear evaluation (Grill et al., 2020).

    python -m adapter --config <resolved.json> --out <dir>

BYOL (Bootstrap Your Own Latent): an online network (ResNet-50 + projector +
predictor) is trained so its prediction of one view matches an EMA **target**
network's projection of the other, under a symmetric negative cosine loss -- no
negatives, no queue (step 1). linear_eval then probes the frozen online backbone
...
predictor and target network are training machinery and are left out.
...
def _model_section(train: dict) -> dict:
    return {"arch": str(train["arch"]),
            "encoder_dim": int(train["encoder_dim"]),
            "proj_hidden_dim": int(train["proj_hidden_dim"]),
            "proj_output_dim": int(train["proj_output_dim"]),
            "pred_hidden_dim": int(train["pred_hidden_dim"]),
            "pred_output_dim": int(train["pred_output_dim"])}
...
def _training_section(train: dict) -> dict:
    return {"epochs": int(train["epochs"]),
            ...
            "trust_coefficient": float(train["trust_coefficient"]),
...
def load_encoder(state_dict: dict, config: dict):
    ...
    if absent:
        raise RuntimeError(
            ...
            "projector, predictor and target network are expected to be "
            "missing; the online backbone is not")
    return model
...
def run_linear_eval(config: dict, out: Path, _run=None) -> dict:
    ...
    if _run is None:
        if str(METHOD_DIR) not in sys.path:
            sys.path.insert(0, str(METHOD_DIR))
        from evaluate_linear_byol import run as _run
...
def _absent_reason(config_path) -> "str | None":
    if _stage_of(config_path) != "linear_eval":
        return None
    return ("this stage fits a linear probe on the frozen BYOL backbone and "
