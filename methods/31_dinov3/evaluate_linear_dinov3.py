"""Linear evaluation for DINOv3: a probe on the frozen backbone.

The representation is the DINOv3 backbone this port trains -- the EMA teacher's
ViT, read through its CLS token (`backbone(x, is_global=True)[0]`, the released
output norm included), one embed_dim feature per image. Images use the
...
from train_pretrain_dinov3 import build_vit, make_deterministic, resolve_device, MODEL_ARGS  # noqa: E402
...
def extract_cls(backbone, imgs, device) -> "torch.Tensor":
    cls, _patches = backbone(imgs.to(device), is_global=True)
    return cls
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="DINOv3 linear eval")
...
def run(args, config: "dict | None" = None, model=None) -> dict:
    ...
    print(f"DINOv3 linear eval  device={device}  embed_dim={train['embed_dim']}  "
          f"img_size={isz}")
