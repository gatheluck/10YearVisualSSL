"""Adapter for siglip: the as-is frozen-backbone linear probe (no pretraining).

    python -m adapter --config <resolved.json> --out <dir>

SigLIP (Zhai, Mustafa, Kolesnikov & Beyer, 2023) is ported **eval-only**: a single
`linear_eval` stage freezes the official sigmoid-loss image-text pretrained image
tower (`vit_base_patch16_siglip_224.webli`) and fits a linear probe on its pooled
image embedding. SigLIP's from-scratch pretraining (the WebLI image-text corpus,
many TPU-days) is the excluded step, so the port reuses the released checkpoint,
exactly as the capture's as-is multimodal comparison (CompEval) does. The probed
representation is a genuine learned feature, so the number is comparable -- the
multimodal sibling of `38_clip`.

There is **no author submodule**: the model class is timm's (a pinned pip
dependency), and the weights are a sha256-pinned download recorded as
...
random tiny SigLIP-style ViT, so nothing is downloaded.

Licence: the SigLIP weights are redistributed by timm under Apache-2.0. Nothing
under it is copied into this repository; the model constructor is imported from the
...
# ── linear_eval config ───────────────────────────────────────────────────────
# The architecture keys let the hermetic smoke build a random tiny SigLIP-style
# ViT; the shipped config pins the official ViT-B/16 SigLIP and names the
# downloaded ckpt.
...
def run_linear_eval(config: dict, out: Path, _run=None) -> dict:
    ...
    if _run is None:
        ...
        from evaluate_linear_siglip import run as _run
