"""Linear evaluation for SigLIP: a probe on the frozen pretrained image tower.

SigLIP's as-is comparison freezes the official sigmoid-loss image-text pretrained
image tower (`vit_base_patch16_siglip_224.webli`) and fits a single linear layer
on its pooled image embedding. The representation is a genuine learned feature, so
the number is comparable (the multimodal "pretrained-backbone reuse" row, the
SigLIP sibling of `38_clip`).
...
empty and builds a **random tiny** SigLIP-style ViT from the config's architecture
keys, so nothing is downloaded and the pipeline runs on a CPU.

SigLIP's timm architecture is a `VisionTransformer` with no class token and a MAP
attention-pooling head (`global_pool='map'`); everything else is the timm default
(GELU, LayerNorm, mlp_ratio 4, qkv_bias) -- so the tiny smoke model is built in
that style, only smaller.
...
def build_model(train: dict, device: "torch.device"):
    """Build the SigLIP image tower from timm and freeze it, `num_classes=0` so it
    yields the pooled pre-classifier feature.

    `ckpt` empty -> a random tiny SigLIP-style `VisionTransformer` built directly
    ...
    `vit_base_patch16_siglip_224`) built with `pretrained=False` and the downloaded
    ...
    if ckpt:
        ...
        # SigLIP's timm style: no class token, a MAP attention-pool head, the
        # timm ViT defaults otherwise -- smaller for the smoke.
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="SigLIP linear eval (frozen image tower)")
...
def run(args, config: "dict | None" = None, model=None) -> dict:
    ...
    print(f"SigLIP linear eval  device={device}  "
          f"backbone={'pretrained' if train.get('ckpt') else 'random (smoke)'}")

    # The normalisation follows the backbone's own data config (SigLIP uses a
    # symmetric 0.5 mean/std, not ImageNet's); the crop resolution is taken from
    # the config so the tiny smoke can shrink it.
    import timm.data
