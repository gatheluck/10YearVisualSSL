"""Linear evaluation for MAE: a probe on the trained encoder.

MAE's downstream representation is the model this port trains: the encoder,
global-average-pooled over patch tokens (He et al. Section 4). So this stage
reads the trained `encoder.pt` and its number is a genuine, comparable linear
probe.

The probe follows the lab's ARSSL protocol: features are extracted once and
cached, mean-centred and L2-normalised, and a single linear layer is trained
with SGD (momentum) under a cosine schedule; top-1 and top-5 are reported.
"""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="MAE linear eval")
...
def run(args, config: "dict | None" = None, mae=None) -> dict:
    ...
    print(f"MAE linear eval  device={device}  pool={train['pool']}")
