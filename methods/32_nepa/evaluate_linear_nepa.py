"""Linear evaluation for NEPA: a probe on the frozen EMA model.

The representation is the EMA NEPA model this port ships as encoder.pt -- read
through ``extract_features(pool="avg")`` (the average of the causal autoregressive
predictor output over positions), one embed_dim feature per image. Images use the
deterministic val pipeline (resize + centre crop, NEPA normalisation, no
...
from train_pretrain_nepa import make_deterministic, resolve_device, MODEL_ARGS  # noqa: E402
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="NEPA linear eval")
...
def run(args, config: "dict | None" = None, model=None) -> dict:
    ...
    print(f"NEPA linear eval  device={device}  embed_dim={train['embed_dim']}  "
          f"img_size={isz}")
