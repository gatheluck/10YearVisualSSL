"""Adapter for 11_cpc, step 1 and linear evaluation (van den Oord et al., 2018).
...
def _model_section(train: dict) -> dict:
    return {"z_dim": int(train["z_dim"]), "c_dim": int(train["c_dim"]),
            "pred_steps": int(train["pred_steps"]),
            "context_layers": int(train["context_layers"]),
            "encoder_width_mult": float(train["encoder_width_mult"])}


def _data_section(train: dict, data_root: str) -> dict:
    return {"data_root": str(data_root),
            "source_size": int(train["source_size"]),
            "img_size": int(train["img_size"]),
            "patch_size": int(train["patch_size"]),
            "patch_crop_size": int(train["patch_crop_size"]),
...
def load_encoder(state_dict: dict, config: dict):
    ...
    if absent:
        raise RuntimeError(
            f"encoder.pt is missing encoder weights: {absent[:5]}. The context "
            "and predictors are expected to be missing; the encoder is not")
    return model
...
def run_linear_eval(config: dict, out: Path, _run=None) -> dict:
    ...
    if _run is None:
        if str(METHOD_DIR) not in sys.path:
            sys.path.insert(0, str(METHOD_DIR))
        from evaluate_linear_cpc2018 import run as _run
...
def _absent_reason(config_path) -> "str | None":
    if _stage_of(config_path) != "linear_eval":
        return None
    return ("this stage fits a linear probe on the frozen CPC patch encoder and "
