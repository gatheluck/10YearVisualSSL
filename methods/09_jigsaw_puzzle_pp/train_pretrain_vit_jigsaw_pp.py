...
scratch) reads the reassembled puzzle image (70% grayscale, 0-2 occlusion tiles,
per-tile normalization) and classifies which of the 701 permutations was applied
(CrossEntropy on the CLS token). AdamW; linear warmup then cosine decay to
`min_lr`; gradient clipping; mixed precision on CUDA. Checkpoints at each
`save_at_epochs` milestone (100/200/300) plus `checkpoint_latest.pth`. The
capture's DDP/torchrun launch is dropped; the device is resolved, not sniffed;
AMP is CUDA-only so the CPU path stays plain fp32.
"""
...
from data.jigsaw_pp_vit_dataset import JigsawPPViTDataset            # noqa: E402
from models.vit_jigsaw_pp import build_vit_jigsaw_pp_model           # noqa: E402
from train_pretrain_jigsaw_pp import (make_deterministic,            # noqa: E402
                                      resolve_device)
...
def run(args, config: "dict | None" = None) -> dict:
    ...
    m = cfg["model"]
    model = build_vit_jigsaw_pp_model(**model_kwargs(m)).to(device)
    model.train()

    dataset = JigsawPPViTDataset(
        ...
        max_occlusions=int(m["max_occlusions"]))
    tr = cfg["training"]
