"""Step-2 unified ViT-B/16 SimCLR v1 pretraining, in one process.
...
`SimCLRDataset` and `NTXentLoss`). Optimiser AdamW (default betas); linear warmup
then cosine decay to `min_lr`; gradient clipping; mixed precision on CUDA.
Checkpoints at each `save_at_epochs` milestone (100/200/300) plus
`checkpoint_latest.pth`. The capture's DDP/torchrun launch and SyncBatchNorm are
dropped, as in every port; the device is resolved, not sniffed; AMP is CUDA-only.
"""
...
from models.vit_simclr import build_vit_simclr                     # noqa: E402
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="SimCLR v1 Step-2 ViT-B/16")
...
def run(args, config: "dict | None" = None) -> dict:
    ...
    m = cfg["model"]
    model = build_vit_simclr(**model_kwargs(m)).to(device)
    model.train()
    ...
    print("=" * 70)
    print("SimCLR v1  pretrain: unified ViT-B/16 (Step 2 protocol, from scratch)")
    ...
    print("\nSimCLR v1 Step-2 ViT pretraining complete!")
