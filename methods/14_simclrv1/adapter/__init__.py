"""Adapter for 14_simclrv1, step 1 and linear evaluation (Chen et al., 2020).

    python -m adapter --config <resolved.json> --out <dir>

SimCLR v1, the paper-faithful ResNet-50 path: two augmented views feed a shared
ResNet-50 + 2-layer MLP projection head; the NT-Xent loss pulls the matching pair
...
MODEL_KEYS = frozenset({"out_dim", "img_size"})
...
def load_encoder(state_dict: dict, config: dict):
    ...
    if train.get("arch", "resnet") == "vit":
        from models.vit_simclr import build_vit_simclr
        from train_pretrain_vit_simclr import model_kwargs as vit_mk
        model = build_vit_simclr(**vit_mk(train))
    else:
...
def run_training(config: dict, out: Path, _run=None) -> dict:
    ...
    if _run is None:
        if str(METHOD_DIR) not in sys.path:
            sys.path.insert(0, str(METHOD_DIR))
        if arch == "vit":
            from train_pretrain_vit_simclr import run as _run
        else:
            from train_pretrain_simclr import run as _run
...
def run_linear_eval(config: dict, out: Path, _run=None) -> dict:
    ...
    if _run is None:
        if str(METHOD_DIR) not in sys.path:
            sys.path.insert(0, str(METHOD_DIR))
        from evaluate_linear_simclr import run as _run
...
def _absent_reason(config_path) -> "str | None":
    if _stage_of(config_path) != "linear_eval":
        return None
    return ("this stage fits a linear probe on the frozen SimCLR v1 backbone and "
