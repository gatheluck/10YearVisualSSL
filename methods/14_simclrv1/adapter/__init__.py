...
STEP1_TRAIN_ONLY = frozenset({"epochs", "batch_size", "num_workers", "lr",
                              ...
                              "warmup_epochs"})
STEP1_TRAIN_KEYS = MODEL_KEYS | LOSS_KEYS | DATA_KEYS | STEP1_TRAIN_ONLY
...
def load_encoder(state_dict: dict, config: dict):
    ...
    model = build_resnet_simclr(**model_config(config["train"]))
...
def run_training(config: dict, out: Path, _run=None) -> dict:
    if _run is None:
        if str(METHOD_DIR) not in sys.path:
            sys.path.insert(0, str(METHOD_DIR))
        from train_step1_simclr import run as _run
