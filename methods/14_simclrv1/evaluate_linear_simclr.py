"""Linear evaluation for SimCLR v1: a probe on the frozen ResNet-50 backbone.

The representation is the model this port trains -- the ResNet-50 backbone read
through `get_encoder()`, one 2048-d feature per image. Images use SimCLR's own
deterministic eval pipeline (bicubic resize + centre crop, **no** ImageNet
mean/std normalisation -- the encoder was trained on unnormalised [0,1] inputs,
so the probe feeds it the same). The probe then follows the lab's shared ARSSL
...
top-1 and top-5 are reported. Using the shared ARSSL head (rather than the
capture's own lr-scaled 90-epoch head) is what makes the number comparable across
the ported methods.
"""
...
from train_pretrain_simclr import make_deterministic, resolve_device   # noqa: E402


def _val_transform(image_size: int):
    resize = int(math.ceil(image_size / 0.875))
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="SimCLR v1 linear eval")
...
def run(args, config: "dict | None" = None, model=None) -> dict:
    ...
    if model is None:
        from models import build_resnet_simclr
        from train_pretrain_simclr import model_config
        model = build_resnet_simclr(**model_config(train))
    encoder = model.get_encoder().to(device)
    encoder.eval()
    for p in encoder.parameters():
        p.requires_grad = False
    print(f"SimCLR v1 linear eval  device={device}  img_size={train['img_size']}")
