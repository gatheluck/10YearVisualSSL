"""Adapter for data2vec2: the as-is frozen-backbone linear probe (no pretraining).

    python -m adapter --config <resolved.json> --out <dir>

data2vec 2.0 (Baevski et al., 2023; arXiv:2212.07525) is ported **eval-only**: a
single `linear_eval` stage freezes the official pretrained vision backbone
(`facebook/data2vec-vision-base`, a BEiT-architecture ViT self-distilled on IN-1k)
and fits a linear probe on its CLS token. data2vec 2.0's from-scratch
self-distillation pretraining (IN-1k, many GPU-days) is the excluded step, so the
port reuses the released checkpoint, exactly as the capture's as-is comparison
does. The probed representation is a genuine SSL feature, so the number is
comparable.

There is **no author submodule**: the model class is `transformers`'
`Data2VecVisionModel` (a pinned pip dependency), and the weights are a
sha256-pinned download recorded as `backbone_artifact` in provenance.json and
fetched by `bin/fetch-weights.py`. This stage trains nothing and produces no
`encoder.pt` -- it records `encoder_absent_reason` and the backbone it read is
named in the config (`train.ckpt`). CI stays hermetic: with an empty `train.ckpt`
the smoke builds a random tiny Data2VecVisionModel, so nothing is downloaded.

Licence: the data2vec-vision weights are redistributed by Meta AI (via
HuggingFace) under Apache-2.0. Nothing under it is copied into this repository; the
model constructor is imported from the pinned transformers dependency.
"""
...
# Data2VecVisionModel; the shipped config pins the official data2vec-vision-base
# and names the downloaded ckpt.
...
def run_linear_eval(config: dict, out: Path, _run=None) -> dict:
    ...
    if _run is None:
        ...
        from evaluate_linear_data2vec2 import run as _run
