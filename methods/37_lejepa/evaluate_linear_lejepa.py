"""Linear evaluation for LeJEPA: a probe on the frozen backbone.

The representation is the LeJEPA backbone this port trains -- one num_features
vector per image (the ViT's CLS token / pooled feature). Images use the
...
(The capture's own LeJEPA eval fits a LayerNorm+Linear head; using the shared
...
EVAL_MODEL_ARGS = ("name", "img_size", "drop_path_rate")
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="LeJEPA linear eval")
...
def run(args, config: "dict | None" = None, model=None) -> dict:
    ...
    if model is None:
        backbone = LeJEPABackbone(build_backbone(
            str(train["name"]), img_size=isz,
            drop_path_rate=float(train["drop_path_rate"])))
    elif isinstance(model, LeJEPABackbone):
        backbone = model
    else:                                     # a full LeJEPAEncoder
        backbone = model.get_encoder()
    backbone = backbone.to(device)
    backbone.eval()
    for p in backbone.parameters():
        p.requires_grad = False
    print(f"LeJEPA linear eval  device={device}  backbone={train['name']}  "
          f"img_size={isz}")
