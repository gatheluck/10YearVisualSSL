# 4_context_encoder — step 1 and linear evaluation
...
```bash
python3 bin/launch.py --config methods/4_context_encoder/configs/step1.yaml --method 4_context_encoder --set DATA_ROOT=/path/to/imagenet
```

Then the linear evaluation, on the `encoder.pt` the first stage wrote:

```bash
python3 bin/launch.py --config methods/4_context_encoder/configs/linear_eval.yaml --method 4_context_encoder --set DATA_ROOT=/path/to/imagenet --set ENCODER=runs/<step1-run>/out/encoder.pt
```
