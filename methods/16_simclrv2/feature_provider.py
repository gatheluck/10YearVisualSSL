"""Feature-extraction provider for 16_simclrv2.
...
wrapper that reuses this method's own pieces, so the knowledge of how SimCLR v2
turns an image into a vector stays in one place:

- the frozen encoder is rebuilt from `encoder.pt` by the adapter's
  `load_encoder`, then read through `get_encoder()` (the 2048-d ResNet-50
  backbone; the 3-layer projection head that SimCLR v2 adds over v1 is left
  behind);
- images go through the method's own deterministic eval pipeline
  (`_build_loader`: bicubic resize + centre crop, [0,1], **no** ImageNet
  mean/std -- SimCLR trained on unnormalised inputs);
...
def extract_val_features(*, encoder_path: str, data_root: str, split: str,
    ...
    ev = importlib.import_module("evaluate_linear_simclrv2")
    ...
    meta = {
        ...
        "preprocessing": ("SimCLR eval: bicubic resize + centre crop, [0,1], "
