"""SimCLR v2 step 1 (Chen et al., 2020), the paper-faithful ResNet-50 path.

A self-contained re-implementation, ported from the lab's own SimCLR v2 code. Two
...
from models import build_resnet_simclrv2        # noqa: E402
from loss import NTXentLoss                    # noqa: E402
from data import SimCLRv2Dataset                 # noqa: E402
from optim import LARS                         # noqa: E402

MODEL_KEYS = ("out_dim", "width_multiplier")


def model_config(model: dict) -> dict:
    """The kwargs build_resnet_simclrv2 needs to rebuild the model for loading.
    out_dim shapes the head; width_multiplier shapes the backbone (resnet50 vs
    wide_resnet50_2), so encoder.pt only loads back into the matching width."""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="SimCLR v2 step 1 (ResNet-50)")
...
def run(args, config: dict | None = None) -> dict:
    ...
    model = build_resnet_simclrv2(
        out_dim=int(m["out_dim"]),
        width_multiplier=int(m["width_multiplier"])).to(device)
    model.train()
    ...
    base_lr = float(t["lr"])
    print("=" * 70)
    print("SimCLR v2  Step 1: ResNet-50 + NT-Xent + LARS  (arXiv:2006.10029)")
    ...
    print("\nSimCLR v2 Step 1 training complete!")
