...
# The unified ViT-B/16 Step-2 path (arch: vit), additive to the native ResNetv2
# path. `arch` (already a model key here, "resnetv2" natively) selects the path;
# the ViT is single-prototype-head (vs the native multi-head), self-labelled by
# per-epoch Sinkhorn under an AdamW/cosine recipe with milestone checkpoints.
ARCHS = ("resnetv2", "vit")
VIT_MODEL_KEYS = frozenset({"arch", "image_size", "patch_size", "embed_dim",
                            "depth", "num_heads", "mlp_ratio", "drop_rate",
                            "attn_drop_rate"})
VIT_CLUSTERING_KEYS = frozenset({"k", "sinkhorn_iters", "sinkhorn_tol",
                                 "lambda", "epsilon", "temperature"})
...
def load_encoder(state_dict: dict, config: dict):
    ...
    if train.get("arch") == "vit":
        from models import build_vit_sela
        from train_pretrain_vit_sela import model_kwargs as vit_mk
        # k shapes only the prototype head (not encoder.*), so it defaults here.
        model = build_vit_sela(**vit_mk(train))
    else:
        from models import ResNetSeLa
        from train_pretrain_sela import model_config
        # The prototype heads are excluded from encoder.pt, so k / num_heads
        # (which only shape them) are free here; the backbone loads back.
        model = ResNetSeLa(num_classes=1, num_heads=1,
                           **model_config(train))
...
def run_training(config: dict, out: Path, _run=None) -> dict:
    ...
    if _run is None:
        if str(METHOD_DIR) not in sys.path:
            sys.path.insert(0, str(METHOD_DIR))
        if arch == "vit":
            from train_pretrain_vit_sela import run as _run
        else:
            from train_pretrain_sela import run as _run
