"""Linear evaluation for SeLa: a probe on the frozen ResNet backbone.

The representation is the model this port trains -- the ResNet backbone read
through `get_features()`, one 2048-d feature per image. Images use the
deterministic val pipeline (resize + centre crop, ImageNet normalisation, no
augmentation -- the same normalisation SeLa trains with). The probe follows the
...
def extract_features(model, loader, device):
    ...
    for imgs, lbs in loader:
        feats.append(model.get_features(imgs.to(device, non_blocking=True))
                     .float().cpu())
        labels.append(lbs)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="SeLa linear eval")
...
def run(args, config: "dict | None" = None, model=None) -> dict:
    ...
    print(f"SeLa linear eval  device={device}  arch={train['arch']}  "
          f"img_size={isz}")
