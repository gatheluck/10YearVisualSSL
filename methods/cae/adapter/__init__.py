"""Adapter for cae: the as-is frozen-backbone linear probe (no pretraining).

    python -m adapter --config <resolved.json> --out <dir>

CAE (Context Autoencoder; Chen et al., 2022; arXiv:2202.04200) is ported
**eval-only**: a single `linear_eval` stage freezes the pretrained vision backbone
(a BEiT-architecture ViT-B/16) and fits a linear probe on its CLS token. CAE's
from-scratch context-autoencoder pretraining (IN-1k, many GPU-days) is the excluded
step, so the port reuses a released checkpoint, exactly as the capture's as-is
comparison does. The probed representation is a genuine SSL feature, so the number
is comparable.

There is **no author submodule**: neither timm nor transformers carries a CAE model
class, so the model is a small self-contained BEiT-style ViT in the method's own
`evaluate_linear_cae.py`, and the weights are a sha256-pinned download recorded as
`backbone_artifact` in provenance.json and fetched by `bin/fetch-weights.py`. The
checkpoint is the OpenMMLab mmselfsup **reproduction** of CAE ViT-B (the capture
logged the official weights as unavailable, deficiency DEF-02; provenance.json states
plainly that this is a reproduction, not the authors' released weights). This stage
trains nothing and produces no `encoder.pt` -- it records `encoder_absent_reason` and
the backbone it read is named in the config (`train.ckpt`). CI stays hermetic: with
an empty `train.ckpt` the smoke builds a random tiny ViT, so nothing is downloaded.

Licence: the OpenMMLab CAE reproduction weights are redistributed by OpenMMLab under
Apache-2.0. Nothing under it is copied into this repository; the model is built from
...
# ── linear_eval config ───────────────────────────────────────────────────────
# The architecture keys let the hermetic smoke build a random tiny CAE ViT; the
# shipped config pins the OpenMMLab CAE ViT-B and names the downloaded ckpt.
...
def run_linear_eval(config: dict, out: Path, _run=None) -> dict:
    ...
    if _run is None:
        ...
        from evaluate_linear_cae import run as _run
