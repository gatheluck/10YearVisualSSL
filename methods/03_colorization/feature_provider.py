"""Feature-extraction provider for 03_colorization.
...
colorization turns an image into a vector stays in one place:

- the frozen encoder is rebuilt from `encoder.pt` by the adapter's
  `load_encoder`, then read through `get_encoder()` (the 512-d
  global-average-pooled CNN trunk);
- images go through the method's own deterministic eval pipeline
  (`_build_loader` -> `ColorizationProbeDataset`: bicubic resize + centre crop,
  RGB->CIE Lab, the **L** channel scaled by 1/100 as a single-channel input,
  **no** ImageNet mean/std -- the encoder reads only lightness);
...
def extract_val_features(*, encoder_path: str, data_root: str, split: str,
    ...
    ev = importlib.import_module("evaluate_linear_colorization")
    ...
    _dataset, loader = ev._build_loader(
        str(data_root), split, image_size, crop_size,
    ...
    meta = {
        ...
        "preprocessing": ("colorization eval: bicubic resize + centre crop, "
                          "RGB->CIE Lab, L channel / 100 as a 1-channel input, "
