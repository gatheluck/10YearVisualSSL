"""Adapter for cosmos3_super: the as-is frozen-backbone linear probe (no pretraining).

    python -m adapter --config <resolved.json> --out <dir>

NVIDIA Cosmos3-Super (nvidia/Cosmos3-Super; github.com/nvidia/cosmos) is ported
**eval-only**: a single `linear_eval` stage freezes the released world-foundation
model's Qwen3-VL **vision encoder** and fits a linear probe on its mean-pooled patch
tokens, exactly as the capture's as-is Step-3 VideoGen comparison does (the 64B
MoT / DiT / VAE are never loaded). Cosmos3-Super's from-scratch pretraining is the
excluded step, so the port reuses the released checkpoint. The probed representation
is a genuine learned feature, so the number is comparable -- the multimodal
"pretrained-backbone reuse" row, the `transformers`-sourced sibling of `data2vec2`
and `sam3`.

There is **no author submodule**: the model class is `transformers`'
`Qwen3VLVisionModel` (a pinned pip dependency), and the weights are a sha256-pinned
download recorded as `backbone_artifact` in provenance.json and fetched by
`bin/fetch-weights.py`. Unlike `sam3`, the released checkpoint IS directly loadable
by the HF class: a real run calls `Qwen3VLVisionModel.from_pretrained` on the
`vision_encoder/` directory (config.json + model.safetensors), so no trunk
conversion is needed. This stage trains nothing and produces no `encoder.pt` -- it
records `encoder_absent_reason` and the backbone it read is named in the config
(`train.ckpt`). CI stays hermetic: with an empty `train.ckpt` the smoke builds a
random tiny `Qwen3VLVisionModel`, so nothing is downloaded.

Licence: the Cosmos3-Super weights are NVIDIA's under OpenMDW-1.1 (public, not
gated). Nothing under it is copied into this repository; the model constructor is
imported from the pinned transformers dependency, and the weights are a
...
# ── linear_eval config ───────────────────────────────────────────────────────
# The architecture keys let the hermetic smoke build a random tiny
# Qwen3VLVisionModel; the shipped config pins the official Cosmos3-Super vision
# encoder and names the downloaded ckpt directory.
...
def run_linear_eval(config: dict, out: Path, _run=None) -> dict:
    ...
    if _run is None:
        ...
        from evaluate_linear_cosmos3_super import run as _run
