"""Linear evaluation for Franca: a probe on the frozen pretrained backbone.

Ported from the lab's `methods/36_franca/evaluate_linear_franca.py`. The
representation is Franca's pretrained ViT-B/14 CLS token
(`forward_features(x)["x_norm_clstoken"]`), frozen; a single linear layer is
fitted on it. Unlike var (which probes a tokeniser), this is a genuine SSL
representation, so the number is comparable -- the capture calls it the "Step 1
caveat / pretrained-backbone reuse" row, analogous to DINOv2.

The model is the pinned upstream under `third_party/franca`, imported not copied.
...
Changed during the port, and recorded in `provenance.json`:
  ...
  - **features are extracted in fp32**; the capture used a bfloat16 autocast,
    which is a GPU speed path with no meaningful effect on a frozen-feature
    linear probe and does not run the same on a CPU or on pre-Ampere GPUs.
  - **RASA is disabled** (`use_rasa_head=False`), as the capture's README notes
    the official RASA loader mismatches ViT-B/14; the CLS probe does not use it.
"""
...
def build_model(official_dir: Path, train: dict, device: "torch.device"):
    """Build the Franca backbone from the pinned upstream and freeze it.

    `ckpt` empty -> `pretrained=False`, a random backbone (the hermetic smoke).
    A path -> the builder loads it via `local_state_dict`, which does the
    upstream's own key handling (strips the teacher/module/backbone prefixes)."""
    ...
    from franca.hub.backbones import (franca_vitb14, franca_vitl14,
                                      franca_vitg14)
    builders = {"franca_vitb14": franca_vitb14,
                "franca_vitl14": franca_vitl14,
                "franca_vitg14": franca_vitg14}
    name = train["name"]
    if name not in builders:
        raise ValueError(f"unsupported Franca model: {name}")
    ckpt = train.get("ckpt") or ""
    model = builders[name](
        pretrained=bool(ckpt),
        weights=str(train["weights"]),
        local_state_dict=(ckpt or None),
        img_size=int(train["resolution"]),
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Franca linear eval (frozen backbone)")
...
def run(args, config: "dict | None" = None, model=None,
    ...
    print(f"Franca linear eval  device={device}  "
