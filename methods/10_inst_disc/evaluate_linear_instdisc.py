"""Linear evaluation for Instance Discrimination: a probe on the frozen backbone.

The representation is the model this port trains -- the ResNet-50 backbone
(`get_encoder()`), which maps an image to a 2048-d feature (the 128-d projection
head is discarded, the standard SSL convention). Images are ImageNet-normalised
and resized to the training size. The probe follows the lab's ARSSL protocol:
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="InstDisc linear eval")
...
def run(args, config: "dict | None" = None, model=None) -> dict:
    ...
    print(f"InstDisc linear eval  device={device}  img_size={train['img_size']}")
