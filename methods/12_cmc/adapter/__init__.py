...
def load_encoder(state_dict: dict, config: dict):
    ...
    if train.get("arch", "alexnet") == "vit":
        from models import build_vit_cmc
        from train_pretrain_vit_cmc import model_kwargs as vit_mk
        model = build_vit_cmc(**vit_mk(train))
    else:
...
def run_training(config: dict, out: Path, _run=None) -> dict:
    ...
    if _run is None:
        if str(METHOD_DIR) not in sys.path:
            sys.path.insert(0, str(METHOD_DIR))
        if arch == "vit":
            from train_pretrain_vit_cmc import run as _run
        else:
