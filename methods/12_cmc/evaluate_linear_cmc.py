"""Linear evaluation for CMC: a probe on the frozen two-branch AlexNet.

The representation is the model this port trains -- the encoder read through
`get_encoder()`, which maps a Lab image to the layer-6 (fc6) features of both
branches, concatenated (2 x 2048). Layer 6 is the paper's best single-branch
layer. Images use the deterministic val pipeline (resize + centre crop, no
...
def _build_loader(data_root: str, split: str, image_size: int, batch_size: int,
                  num_workers: int):
    dataset = CMCDataset(str(Path(data_root) / split), mode="val",
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Visual CMC linear eval")
...
def run(args, config: "dict | None" = None, model=None) -> dict:
    ...
    print(f"Visual CMC linear eval  device={device}  "
