"""Linear evaluation for DINO: a probe on the frozen teacher ViT backbone.

The representation is the teacher ViT this port ships as encoder.pt -- read
through its CLS feature (embed_dim), one feature per image. Images use the
deterministic val pipeline (resize + centre crop, ImageNet normalisation, no
augmentation). The probe follows the lab's ARSSL protocol: features are extracted
once and cached, mean-centred and L2-normalised, and a single linear layer is
trained with SGD (momentum) under a cosine schedule; top-1 and top-5 are reported.

The capture's own DINO linear eval concatenates the last-4 blocks' CLS tokens and
trains a distributed head; this port uses the shared ARSSL single-feature probe
instead, so the number is comparable across the ported methods (a documented
deviation, the same as every other port).
"""
...
from train_pretrain_dino import make_deterministic, resolve_device   # noqa: E402
...
def _train_transform(image_size: int):
    ...
    # probe_transforms, then DINO's own ImageNet normalisation tail (bicubic to
    # match this method's eval interpolation).
...
def extract_features(backbone, loader, device):
    ...
    for imgs, lbs in loader:
        feats.append(backbone(imgs.to(device, non_blocking=True)).float().cpu())
        labels.append(lbs)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="DINO linear eval")
...
def run(args, config: "dict | None" = None, model=None) -> dict:
    ...
    if model is None:
        ...
        model = build_vit(str(train["arch"]), img_size=isz)
    backbone = model.to(device)
    backbone.eval()
    for p in backbone.parameters():
        p.requires_grad = False
    print(f"DINO linear eval  device={device}  arch={train['arch']}  "
          f"img_size={isz}")
