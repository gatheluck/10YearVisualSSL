...
to TensorBoard; none is needed for a single-process run, so `train_step1_dino.py`
...
## Running
    ...
    python bin/resolve-config.py methods/23_dino/configs/step1.yaml \
    ...
    python bin/resolve-config.py methods/23_dino/configs/linear_eval.yaml \
