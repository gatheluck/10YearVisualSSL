"""Linear evaluation for EVA-02: a probe on the frozen pretrained backbone.

EVA-02's as-is comparison freezes the official masked-image-modelling pretrained
ViT (`eva02_base_patch14_224.mim_in22k`) and fits a single linear layer on its
pooled features. The representation is a genuine SSL feature, so the number is
comparable (the "pretrained-backbone reuse" row, analogous to DINOv2 / Franca).
...
empty and builds a **random tiny** EVA-02 from the config's architecture keys, so
nothing is downloaded and the pipeline runs on a CPU.
...
def build_model(train: dict, device: "torch.device"):
    """Build the EVA-02 backbone from timm and freeze it, `num_classes=0` so it
    yields the pooled pre-classifier feature.

    `ckpt` empty -> a random tiny EVA-02 built directly from the config's
    architecture keys (the hermetic smoke); nothing is downloaded. A path -> the
    named timm architecture (`train["name"]`, e.g. `eva02_base_patch14_224`)
    built with `pretrained=False` and the downloaded checkpoint loaded into it."""
    import timm
    ...
    if ckpt:
        ...
        grid = int(train["img_size"]) // int(train["patch_size"])
        model = Eva(
            img_size=int(train["img_size"]), patch_size=int(train["patch_size"]),
            embed_dim=int(train["embed_dim"]), depth=int(train["depth"]),
            num_heads=int(train["num_heads"]), num_classes=0,
            mlp_ratio=4 * 2 / 3, swiglu_mlp=True, scale_mlp=True,
            qkv_fused=False, use_rot_pos_emb=True, ref_feat_shape=(grid, grid))
    model.to(device)
    model.eval()
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="EVA-02 linear eval (frozen backbone)")
...
def run(args, config: "dict | None" = None, model=None) -> dict:
    ...
    print(f"EVA-02 linear eval  device={device}  "
          f"backbone={'pretrained' if train.get('ckpt') else 'random (smoke)'}")

    # The normalisation follows the backbone's own data config (EVA-02 uses a
