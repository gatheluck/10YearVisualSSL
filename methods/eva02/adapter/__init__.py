"""Adapter for eva02: the as-is frozen-backbone linear probe (no pretraining).

    python -m adapter --config <resolved.json> --out <dir>

EVA-02 (Fang et al., 2023) is ported **eval-only**: a single `linear_eval` stage
freezes the official masked-image-modelling pretrained backbone
(`eva02_base_patch14_224.mim_in22k`) and fits a linear probe on its pooled
features. EVA-02's from-scratch MIM pretraining (IN-22k, many GPU-days) is the
excluded step, so the port reuses the released checkpoint, exactly as the capture's
as-is comparison does. The probed representation is a genuine SSL feature, so the
...
random tiny EVA-02, so nothing is downloaded.

Licence: the EVA-02 weights are redistributed by timm under MIT. Nothing under it
is copied into this repository; the model constructor is imported from the pinned
timm dependency.
"""
...
# ── linear_eval config ───────────────────────────────────────────────────────
# The architecture keys let the hermetic smoke build a random tiny EVA-02; the
# shipped config pins the official EVA-02-B/14 and names the downloaded ckpt.
...
def run_linear_eval(config: dict, out: Path, _run=None) -> dict:
    ...
    if _run is None:
        ...
        from evaluate_linear_eva02 import run as _run
