...
def extract_val_features(*, encoder_path: str, data_root: str, split: str,
    ...
    if ev._is_mnist(str(data_root)):
        ...
        dataset = datasets.ImageFolder(os.path.join(str(data_root), split),
                                       transform=tf)
    provider_support.configure_native_resize(tf, native_options)
    ...
    meta = {
        ...
        "preprocessing": (f"VAE eval: resize to {native_options.get('resize_short_side', image_size)}, "
                          f"centre crop {image_size}, [0,1], no ImageNet "
