"""Linear evaluation for PIRL: a probe on the frozen ResNet-50 trunk.

The representation is the ResNet-50 trunk this port trains -- read through
``get_encoder()`` (the avg-pooled 2048-d backbone feature), one feature per image.
Images use the deterministic val pipeline (resize + centre crop, ImageNet
...
from train_pretrain_pirl import make_deterministic, resolve_device   # noqa: E402
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="PIRL linear eval")
...
def run(args, config: "dict | None" = None, model=None) -> dict:
    ...
    backbone = model.get_encoder().to(device)
    backbone.eval()
    for p in backbone.parameters():
        p.requires_grad = False
    isz = int(train["image_size"])
    print(f"PIRL linear eval  device={device}  arch={train['arch']}  "
          f"img_size={isz}")
