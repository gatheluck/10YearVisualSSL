"""Linear evaluation for MSN: a probe on the frozen backbone.

The representation is the MSN anchor ViT this port trains -- read through the trunk
(`forward_features`, the CLS token at embed_dim; the projection head is left as the
default Identity and excluded from encoder.pt). Images use the deterministic val
pipeline (resize + centre crop, ImageNet normalisation, no augmentation). The probe
follows the lab's ARSSL protocol: features are extracted once and cached,
mean-centred and L2-normalised, and a single linear layer is trained with SGD
(momentum) under a cosine schedule; top-1 and top-5 are reported. (The capture's
own MSN eval runs the official cyanure logistic-regression probe; the shared
single-feature probe is used instead, a documented deviation.)
"""
...
from train_pretrain_msn import make_deterministic, resolve_device, MODEL_ARGS  # noqa: E402
from data import val_transform                                             # noqa: E402
from models import build_msn_backbone                                      # noqa: E402

BACKBONE_ARGS = ("img_size", "patch_size", "embed_dim", "depth", "num_heads",
...
@torch.no_grad()
def extract_cls(backbone, imgs, device) -> "torch.Tensor":
    h, _z = backbone(imgs.to(device, non_blocking=True), return_before_head=True)
    return h
...
def extract_features(backbone, loader, device):
    ...
    for imgs, lbs in loader:
        feats.append(extract_cls(backbone, imgs, device).float().cpu())
        labels.append(lbs)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="MSN linear eval")
...
def run(args, config: "dict | None" = None, model=None) -> dict:
    ...
    if model is None:
        model = build_msn_backbone(**{k: train[k] for k in BACKBONE_ARGS})
    backbone = model.to(device)
    backbone.eval()
    for p in backbone.parameters():
        p.requires_grad = False
    print(f"MSN linear eval  device={device}  embed_dim={train['embed_dim']}  "
          f"img_size={isz}")
