...
torch / torchvision / numpy / PyYAML **+ faiss-gpu** (CUDA lock only).
`requirements.lock.cu130.txt` is the runnable GPU closure (adds `faiss-gpu` and
the nvidia CUDA wheels). `requirements.lock.txt` is the torch-only CPU closure
(no faiss — faiss-gpu has no cross-platform wheel).
...
## Running
    ...
    python bin/resolve-config.py methods/07_deepcluster/configs/step1.yaml \
    ...
    python bin/resolve-config.py methods/07_deepcluster/configs/linear_eval.yaml \
