...
STEP1_TRAIN_ONLY = frozenset({"epochs", "batch_size", "feat_batch_size",
...
# The AlexNet-BN backbone. The reset-each-epoch top_layer and the fixed Sobel
# front-end are excluded.
ENCODER_PREFIXES = ("features.", "classifier.")
...
def load_encoder(state_dict: dict, config: dict):
    ...
    from train_step1_deepcluster import model_config
    model = build_alexnet_deepcluster(**model_config(config["train"]))
...
def run_training(config: dict, out: Path, _run=None) -> dict:
    if _run is None:
        if str(METHOD_DIR) not in sys.path:
            sys.path.insert(0, str(METHOD_DIR))
        from train_step1_deepcluster import run as _run
