"""Linear evaluation for DeepCluster: a probe on the frozen AlexNet-BN backbone.

The representation is the model this port trains -- the backbone read through
`get_features()`, one 4096-d fc7 feature per image (the frozen backbone reads the
RGB image and applies its Sobel front-end). The probe follows the lab's ARSSL
...
def _build_loader(data_root: str, split: str, crop_size: int, batch_size: int,
                  num_workers: int):
    dataset = build_base_dataset(str(Path(data_root) / split),
...
def extract_features(model, loader, device):
    ...
    for imgs, lbs in loader:
        f = model.get_features(imgs.to(device, non_blocking=True))
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="DeepCluster linear eval")
