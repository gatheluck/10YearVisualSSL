"""Linear evaluation for Split-Brain: a probe on the frozen cross-channel encoders.

The representation is the model this port trains -- both branch encoders read
through `extract_features(l, ab)`, which concatenates their spatially-averaged
features (256 + 256 = 512-d). The frozen encoders read the deterministically
cropped L and ab channels. The probe follows the lab's ARSSL protocol: features
are extracted once and cached, mean-centred and L2-normalised, and a single
linear layer is trained with SGD (momentum) under a cosine schedule; top-1 and
top-5 are reported.
"""
...
def _build_loader(data_root: str, split: str, crop_size: int, batch_size: int,
                  num_workers: int):
    dataset = SplitBrainProbeDataset(str(Path(data_root) / split),
                                     crop_size=crop_size)
...
def extract_features(model, loader, device):
    feats, labels = [], []
    for l_input, ab_input, lbs in loader:
        f = model.extract_features(l_input.to(device, non_blocking=True),
                                   ab_input.to(device, non_blocking=True))
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Split-Brain linear eval")
...
def run(args, config: "dict | None" = None, model=None) -> dict:
    ...
    print(f"Split-Brain linear eval  device={device}  "
