"""Adapter for 08_split_brain, step 1 and linear evaluation (Zhang et al., 2017).

    python -m adapter --config <resolved.json> --out <dir>

Split-Brain Autoencoders, the AlexNet path: two cross-channel branches predict
...
def load_encoder(state_dict: dict, config: dict):
    ...
    if absent:
        raise RuntimeError(
            f"encoder.pt is missing encoder weights: {absent[:5]}. The decoders "
            "are expected to be missing; the encoders are not")
    return model
...
def run_linear_eval(config: dict, out: Path, _run=None) -> dict:
    ...
    if _run is None:
        if str(METHOD_DIR) not in sys.path:
            sys.path.insert(0, str(METHOD_DIR))
        from evaluate_linear_split_brain import run as _run
...
def _absent_reason(config_path) -> "str | None":
    if _stage_of(config_path) != "linear_eval":
        return None
    return ("this stage fits a linear probe on the frozen Split-Brain encoders "
