"""Step-2 unified ViT-B/16 MoCo v1 pretraining, in one process.

A faithful port of the capture's `train_step2_vit.py`: a ViT-B/16 (timm, from
scratch) query/key encoder with a momentum queue is trained with InfoNCE on two
augmented views (reusing the port's `MoCoDataset`). Optimiser AdamW with
betas (0.9, 0.95); linear warmup then cosine decay to `min_lr`. Checkpoints at
each `save_at_epochs` milestone (100/200/300) plus `checkpoint_latest.pth`. The
capture's DDP/torchrun launch and TensorBoard are dropped, as in every port; the
device is resolved, not sniffed. Matching the capture's ViT MoCo loop, there is
no AMP and no gradient clipping.
"""
...
from models.vit_moco import build_moco_vit                        # noqa: E402
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="MoCo v1 Step-2 ViT-B/16")
...
def run(args, config: "dict | None" = None) -> dict:
    ...
    m = cfg["model"]
    mo = cfg["moco"]
    model = build_moco_vit(queue_size=int(mo["queue_size"]),
    ...
    print("=" * 70)
    print("MoCo v1  pretrain: unified ViT-B/16 (Step 2 protocol, from scratch)")
    ...
    print("\nMoCo v1 Step-2 ViT pretraining complete!")
