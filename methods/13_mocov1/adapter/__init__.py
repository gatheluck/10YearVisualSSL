"""Adapter for 13_mocov1, step 1 and linear evaluation (He et al., 2019).

    python -m adapter --config <resolved.json> --out <dir>

Momentum Contrast v1, the paper-faithful ResNet-50 path: two augmented views feed
a query encoder and a momentum key encoder; an InfoNCE loss contrasts the query
against the matching key and a FIFO queue of K past keys (step 1). linear_eval
then probes the frozen query ResNet-50 backbone (2048-d). A self-contained
re-implementation (the lab's own code) -- no submodule. The capture's step 2
(ViT) is excluded, as in every port.

`encoder.pt` is the query ResNet-50 backbone (`encoder_q.backbone.*`); the 128-d
projection head, the momentum key encoder and the queue are training machinery
and are left out. `linear_eval` reads this `encoder.pt`; the representation is the
...
# path. The MoCo knobs (queue_size, key_momentum, temperature) are shared; the
# ViT adds its own model dimensions and an AdamW/cosine training recipe with
# milestone checkpoints.
ARCHS = ("resnet", "vit")
VIT_MODEL_KEYS = frozenset({"feature_dim", "img_size", "patch_size",
                            "embed_dim", "depth", "num_heads", "mlp_ratio",
                            "drop_rate", "attn_drop_rate"})
PRETRAIN_VIT_ONLY = frozenset({"epochs", "batch_size", "num_workers", "lr",
                               "weight_decay", "warmup_epochs", "min_lr",
                               "save_at_epochs"})
PRETRAIN_VIT_KEYS = VIT_MODEL_KEYS | MOCO_KEYS | PRETRAIN_VIT_ONLY
...
# The query ResNet-50 backbone. The projection head, key encoder and queue are
# excluded.
...
def load_encoder(state_dict: dict, config: dict):
    ...
    if train.get("arch", "resnet") == "vit":
        from models import build_moco_vit
        from train_pretrain_vit_mocov1 import model_kwargs as vit_mk
        model = build_moco_vit(**vit_mk(train))
    else:
    ...
    if absent:
        raise RuntimeError(
            ...
            "key encoder and queue are expected to be missing; the query "
            "backbone is not")
    return model
...
def run_training(config: dict, out: Path, _run=None) -> dict:
    ...
    if _run is None:
        if str(METHOD_DIR) not in sys.path:
            sys.path.insert(0, str(METHOD_DIR))
        if arch == "vit":
            from train_pretrain_vit_mocov1 import run as _run
        else:
            from train_pretrain_mocov1 import run as _run
...
def run_linear_eval(config: dict, out: Path, _run=None) -> dict:
    ...
    if _run is None:
        if str(METHOD_DIR) not in sys.path:
            sys.path.insert(0, str(METHOD_DIR))
        from evaluate_linear_mocov1 import run as _run
...
def _absent_reason(config_path) -> "str | None":
    if _stage_of(config_path) != "linear_eval":
        return None
    return ("this stage fits a linear probe on the frozen MoCo v1 backbone and "
