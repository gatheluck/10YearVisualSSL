"""ResNet-50 encoder for MoCo v1 (He et al., 2019), ported from the lab's own
paper-faithful implementation.

Two augmented views of an image feed a **query encoder** (ResNet-50 + a single
Linear(2048, feature_dim) projection, L2-normalised -- no MLP, that is v2) and a
**momentum key encoder** (an EMA copy of the query, no gradient). An InfoNCE loss
contrasts the query against the matching key (the positive) and a FIFO **queue**
of K past keys (the negatives).
...
class ResNetEncoder(nn.Module):
    """ResNet-50 trunk with a single linear projection to feature_dim."""
...
class MoCoResNet(nn.Module):
    """MoCo v1 with a ResNet-50 encoder (He et al., 2019)."""
...
def build_moco_resnet(feature_dim: int = 128, queue_size: int = 65536,
                      momentum: float = 0.999,
                      temperature: float = 0.07) -> MoCoResNet:
    return MoCoResNet(feature_dim=feature_dim, queue_size=queue_size,
