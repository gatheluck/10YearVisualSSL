"""Linear evaluation for MoCo v1: a probe on the frozen ResNet-50 backbone.
...
trained with SGD (momentum) under a cosine schedule; top-1 and top-5 are
reported.
"""
...
def _train_transform(image_size: int):
    ...
    # probe_transforms, then MoCo v1's own ImageNet normalisation tail.
    import probe_transforms
    return probe_transforms.basic5_train_transform(
        image_size, normalize=transforms.Normalize(IMAGENET_MEAN, IMAGENET_STD))
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="MoCo v1 linear eval")
...
def run(args, config: "dict | None" = None, model=None) -> dict:
    ...
    print(f"MoCo v1 linear eval  device={device}  img_size={train['img_size']}")
