...
none is needed for a single-process run, so `train_step1_mocov1.py` owns a thin
...
## Running
    ...
    python bin/resolve-config.py methods/13_mocov1/configs/step1.yaml \
    ...
    python bin/resolve-config.py methods/13_mocov1/configs/linear_eval.yaml \
