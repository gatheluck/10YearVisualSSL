...
## Running
    ...
    python bin/resolve-config.py methods/24_beit/configs/step1.yaml \
        --set DATA_ROOT=/path/to/imagenet \
        --set TOKENIZER_CKPT=/path/to/encoder.pkl > resolved.json
    cd methods/24_beit && PYTHONPATH="$PWD/../.." \
        python -m adapter --config /path/to/resolved.json --out /path/to/s1

    # linear eval: DATA_ROOT has train/ and val/; ENCODER is step 1's encoder.pt
    python bin/resolve-config.py methods/24_beit/configs/linear_eval.yaml \
        --set DATA_ROOT=/path/to/imagenet \
