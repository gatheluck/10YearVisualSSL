"""Linear evaluation for BEiT: a probe on the frozen backbone.

The representation is the BEiT backbone this port trains -- read through
``get_encoder()`` (the mean of the patch tokens, CLS excluded), one embed_dim
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="BEiT linear eval")
...
def run(args, config: "dict | None" = None, model=None) -> dict:
    ...
    print(f"BEiT linear eval  device={device}  embed_dim={train['embed_dim']}  "
          f"img_size={isz}")
