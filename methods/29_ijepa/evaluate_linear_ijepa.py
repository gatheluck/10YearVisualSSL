"""Linear evaluation for I-JEPA: a probe on the frozen target ViT encoder.

The representation is the target ViT this port ships as encoder.pt -- read through
its mean-pooled patch tokens (embed_dim), one feature per image (I-JEPA has no CLS
token). Images use the deterministic val pipeline (resize + centre crop, ImageNet
normalisation, no augmentation). The probe follows the lab's ARSSL protocol:
...
from train_pretrain_ijepa import make_deterministic, resolve_device   # noqa: E402
...
def extract_features(backbone, loader, device):
    ...
    for imgs, lbs in loader:
        feats.append(
            backbone.forward_features(imgs.to(device, non_blocking=True))
            .float().cpu())
        labels.append(lbs)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="I-JEPA linear eval")
...
def run(args, config: "dict | None" = None, model=None) -> dict:
    ...
    if model is None:
        ...
        model = build_ijepa_encoder(str(train["name"]), img_size=isz,
                                    patch_size=int(train["patch_size"]))
    backbone = model.to(device)
    backbone.eval()
    for p in backbone.parameters():
        p.requires_grad = False
    print(f"I-JEPA linear eval  device={device}  arch={train['name']}  "
          f"img_size={isz}")
