"""Linear evaluation for SimMIM: a probe on the frozen Swin encoder.

The representation is the Swin encoder this port trains -- read through its
mean-pooled ``forward_features`` (encoder_dim), one feature per image. Images use
the deterministic val pipeline (resize + centre crop, ImageNet normalisation, no
...
The capture's own SimMIM eval standardises features (zero mean / unit std) then
fits a linear head; this port uses the shared single-feature ARSSL probe instead,
so the number is comparable across the ported methods (a documented deviation).
"""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="SimMIM linear eval")
