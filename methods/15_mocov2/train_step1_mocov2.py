"""MoCo v2 step 1 (Chen et al., 2020), the paper-faithful ResNet-50 path.

A self-contained re-implementation, ported from the lab's own MoCo v2 code. Two
...
MoCo v2 differs from v1 by a 2-layer MLP head (in the model), Gaussian-blur
augmentation (in the data module), and a **cosine** LR schedule (here).
...
but inert single-process). `encoder.pt` is the query ResNet-50 backbone; the MLP
head, the key encoder and the queue are excluded.
"""
...
from models import build_mocov2_resnet        # noqa: E402
...
def model_config(model: dict) -> dict:
    """The kwargs build_mocov2_resnet needs to rebuild the encoder for loading.
...
def cosine_lr_with_warmup(base_lr: float, epoch: int, total_epochs: int,
    ...
    """Cosine annealing with linear warmup -- MoCo v2's schedule (v1 used step
    decay)."""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="MoCo v2 step 1 (ResNet-50)")
