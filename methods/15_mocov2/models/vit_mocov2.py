"""ViT Step-2 MoCo v2 model: a unified ViT-B/16 query/key encoder + a momentum
queue, with a 2-layer MLP projection head.

Faithful to the capture's `models/vit_mocov2.py`: a timm `VisionTransformer`
(from scratch) reads the image, its CLS token feeds a 2-layer MLP projector
(`Linear(768, 768) -> ReLU -> Linear(768, feature_dim)`, the MoCo v2 head),
L2-normalised. A momentum key encoder (an EMA copy, no gradient) and a FIFO queue
of K past keys give the InfoNCE negatives; ``forward`` returns
``(loss, logits, labels)`` exactly as the ResNet path does. Restructured to this
port's convention: the ViT trunk lives under ``encoder_q.backbone``
(num_classes=0), so `encoder.pt` keeps only ``encoder_q.backbone.*`` and
...
class ViTEncoderV2(nn.Module):
    """ViT trunk (CLS token) with a 2-layer MLP projection (MoCo v2 style)."""
...
class MoCoV2ViT(nn.Module):
    """MoCo v2 with a ViT-B/16 encoder: momentum encoder, FIFO queue, InfoNCE."""
    ...
    def __init__(self, feature_dim: int = 128, queue_size: int = 65536,
        ...
        self.encoder_q = ViTEncoderV2(**vit_kwargs)
...
def build_mocov2_vit(feature_dim: int = 128, queue_size: int = 65536,
                     momentum: float = 0.999, temperature: float = 0.07,
                     image_size: int = 224, patch_size: int = 16,
                     embed_dim: int = 768, depth: int = 12, num_heads: int = 12,
                     mlp_ratio: float = 4.0, drop_rate: float = 0.0,
                     attn_drop_rate: float = 0.0) -> MoCoV2ViT:
    return MoCoV2ViT(feature_dim=feature_dim, queue_size=queue_size,
