"""Linear evaluation for MoCo v2: a probe on the frozen ResNet-50 backbone.

The representation is the model this port trains -- the query ResNet-50 backbone
read through `get_encoder()`, one 2048-d feature per image. Images use the
deterministic val pipeline (resize + centre crop, ImageNet normalisation, no
augmentation -- the same normalisation MoCo v2 trains with). The probe follows the
lab's ARSSL protocol: features are extracted once and cached, mean-centred and
L2-normalised, and a single linear layer is trained with SGD (momentum) under a
cosine schedule; top-1 and top-5 are reported.
"""
...
from train_step1_mocov2 import make_deterministic, resolve_device   # noqa: E402
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="MoCo v2 linear eval")
...
def run(args, config: "dict | None" = None, model=None) -> dict:
    ...
    if model is None:
        ...
        from train_step1_mocov2 import model_config
        model = build_mocov2_resnet(**model_config(train))
    encoder = model.get_encoder().to(device)
    encoder.eval()
    for p in encoder.parameters():
        p.requires_grad = False
    print(f"MoCo v2 linear eval  device={device}  img_size={train['img_size']}")
