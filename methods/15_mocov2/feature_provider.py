"""Feature-extraction provider for 15_mocov2.
...
wrapper that reuses this method's own pieces, so the knowledge of how MoCo v2
turns an image into a vector stays in one place:

- the frozen encoder is rebuilt from `encoder.pt` by the adapter's
  `load_encoder`, then read through `get_encoder()` (the 2048-d query ResNet-50
  backbone);
- images go through the method's own deterministic eval pipeline
  ...
  MoCo v2 trains on ImageNet-normalised inputs);
...
def extract_val_features(*, encoder_path: str, data_root: str, split: str,
    ...
    ev = importlib.import_module("evaluate_linear_mocov2")
    ...
    meta = {
        ...
        "preprocessing": ("MoCo v2 eval: resize + centre crop, [0,1], "
