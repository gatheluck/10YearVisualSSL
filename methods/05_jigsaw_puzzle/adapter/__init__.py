"""Adapter for 05_jigsaw_puzzle, step 1 and linear evaluation (Noroozi & Favaro,
ECCV 2016).

    python -m adapter --config <resolved.json> --out <dir>

Jigsaw solves a pretext of predicting which permutation reordered an image's 3x3
tiles (step 1), then a linear probe on the shared CFN encoder (linear_eval). A
...
# Selected by the optional `arch` key; absent/`alexnet` is the native CFN path,
# unchanged. The 9 permuted tiles are reassembled into one puzzle image.
ARCHS = ("alexnet", "vit")
...
PRETRAIN_VIT_KEYS = VIT_MODEL_KEYS | VIT_DATA_KEYS | PRETRAIN_VIT_ONLY
...
# The shared CFN encoder. The permutation classifier is excluded.
...
def load_encoder(state_dict: dict, config: dict):
    ...
    if train.get("arch", "alexnet") == "vit":
        from models.vit_jigsaw import build_vit_jigsaw_model
        from train_pretrain_vit_jigsaw import model_kwargs as vit_model_kwargs
        model = build_vit_jigsaw_model(**vit_model_kwargs(train))
    else:
...
def run_training(config: dict, out: Path, _run=None) -> dict:
    ...
    if _run is None:
        if str(METHOD_DIR) not in sys.path:
            sys.path.insert(0, str(METHOD_DIR))
        if arch == "vit":
            from train_pretrain_vit_jigsaw import run as _run
        else:
            from train_pretrain_jigsaw import run as _run
...
def run_linear_eval(config: dict, out: Path, _run=None) -> dict:
    ...
    if _run is None:
        if str(METHOD_DIR) not in sys.path:
            sys.path.insert(0, str(METHOD_DIR))
        from evaluate_linear_jigsaw import run as _run
...
def _absent_reason(config_path) -> "str | None":
    if _stage_of(config_path) != "linear_eval":
        return None
    return ("this stage fits a linear probe on the frozen CFN encoder and "
