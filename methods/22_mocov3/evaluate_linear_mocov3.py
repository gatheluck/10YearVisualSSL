"""Linear evaluation for MoCo v3: a probe on the frozen base ViT trunk.

The representation is the model this port trains -- the base ViT read through
`get_backbone()` (the CLS feature before the projector head), one embed_dim
feature per image. Images use the deterministic val pipeline (resize + centre
crop, ImageNet normalisation, no augmentation). The probe follows the lab's ARSSL
...
from train_pretrain_mocov3 import make_deterministic, resolve_device   # noqa: E402
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="MoCo v3 linear eval")
...
def run(args, config: "dict | None" = None, model=None) -> dict:
    ...
    print(f"MoCo v3 linear eval  device={device}  arch={train['arch']}  "
          f"img_size={isz}")
