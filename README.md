...
| `07_deepcluster` | DeepCluster — Caron et al., ECCV 2018 | step 1 + linear eval | a self-contained re-implementation (the lab's own AlexNet-BN, no submodule); each epoch, fc7 features are PCA-whitened and **k-means** clustered into pseudo-labels the network predicts; `encoder.pt` is the backbone (top_layer excluded); `linear_eval` probes fc7 (4096-d). **The one GPU / x86_64-linux-only method**: clustering uses **faiss** (the paper-target backend), whose wheel has no cross-platform build, so faiss is in the CUDA lock only |
