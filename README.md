...
| `methods/` | **thirty-four methods ported and tested** (thirty-two with a linear evaluation; `36_franca` and `28_dinov2` are eval-only ports, with no step 1). The per-method table is below under [Methods](#methods) |
...
Thirty-one produce **comparable** `linear_probe` accuracy on a genuinely learned
