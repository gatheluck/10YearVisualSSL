...
| `methods/` | **thirty-eight methods ported and tested** (thirty-six with a linear evaluation; `36_franca`, `28_dinov2` and `30_aim` are eval-only ports, with no step 1). The per-method table is below under [Methods](#methods) |
...
Thirty-five produce **comparable** `linear_probe` accuracy on a genuinely learned
