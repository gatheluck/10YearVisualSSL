...
| `methods/` | **twenty-six methods ported and tested** (twenty-four with a linear evaluation; `36_franca` is the first eval-only port, with no step 1). The per-method table is below under [Methods](#methods) |
...
Twenty-three produce **comparable** `linear_probe` accuracy on a genuinely learned
