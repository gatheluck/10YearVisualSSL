...
| `methods/` | **thirty-nine methods ported and tested** (thirty-seven with a linear evaluation; `36_franca`, `28_dinov2` and `30_aim` are eval-only ports, with no step 1). The per-method table is below under [Methods](#methods) |
...
Thirty-six produce **comparable** `linear_probe` accuracy on a genuinely learned
