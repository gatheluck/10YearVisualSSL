...
| `methods/` | **thirty-five methods ported and tested** (thirty-three with a linear evaluation; `36_franca` and `28_dinov2` are eval-only ports, with no step 1). The per-method table is below under [Methods](#methods) |
...
Thirty-two produce **comparable** `linear_probe` accuracy on a genuinely learned
