...
| `32_nepa` | NEPA — Xu et al., 2025 | step 1 + linear eval | a self-contained re-implementation (the lab's own NEPA, no submodule); **next-embedding predictive autoregression** — patch embeddings run through a **causal** transformer predictor, trained by a **negative-cosine** loss against a **stop-gradient** shifted (next-patch) target; an EMA copy is kept for eval; `encoder.pt` is the **EMA** model (`ema_model.*`; the online model excluded); `linear_eval` probes its avg-pooled predictor output (embed_dim), a genuine comparable number. The repo's **first 2D-RoPE ViT** (with QK-norm + LayerScale) and first autoregressive latent objective. NEPA ships **its own ViT** (not `timm`) and trains from scratch on ImageNet, so this port is **torch-only**. The ViT step 2 is excluded |
