...
class TestAStep1Smoke(Base):
    def run_adapter(self, **over):
        ...
        # step1 reads the train/ subdirectory of data_root
        c = self.config(**over)
...
BACKBONE_DIM = 768  # DINOv3 ViT-B/16 embed_dim; the teacher CLS token, one
#                     embed_dim vector per image (the shipped linear_eval config)
...
class TestFeatureProvider(Base):
    ...
    768-d DINOv3 teacher CLS-token feature -- raw, before the probe's normalise
    -- one row per val image, with honest meta.

    The encoder.pt is built from the *shipped* linear_eval config's
    architecture (the provider reads that config). Unlike a plain-ViT method,
    DINOv3's encoder.pt is the teacher backbone with its `backbone.` prefix
    stripped, so the round-trip here rebuilds the backbone with `build_vit`,
    prefixes its state dict the way the teacher_state_dict is laid out, and runs
    the same `adapter.extract_encoder` filter the adapter writes with -- random
    weights do not affect the shape-and-plumbing this proves. Modules load
    ...
    def _make_encoder(self, cfg: dict) -> Path:
        import torch
        trainer = load("dinov3_trainer", METHOD / "train_pretrain_dinov3.py")
        train = cfg["train"]
        model = trainer.build_vit(**{k: train[k] for k in trainer.MODEL_ARGS})
        teacher_like = {f"backbone.{k}": v
                        for k, v in model.state_dict().items()}
        state = adapter.extract_encoder(teacher_like)
        encoder_pt = self.tmp / "encoder.pt"
        torch.save(state, encoder_pt)
        return encoder_pt

    def _provider(self):
        return load("dinov3_feature_provider", METHOD / "feature_provider.py")

    @needs_deps
    def test_it_returns_raw_768d_features_one_per_val_image(self):
        prov_path = METHOD / "feature_provider.py"
        if not prov_path.is_file():
            self.skipTest("31_dinov3 provider not yet present")
        import numpy as np
        ...
        self.assertEqual(feats.shape[1], BACKBONE_DIM,
                         "DINOv3 teacher CLS-token feature is 768-d")
