...
class TestFeatureProvider(Base):
    ...
    512-d global-average-pooled CNN trunk feature -- raw, before the probe's
    ...
    def _make_encoder(self, cfg: dict) -> Path:
        import torch
        models = load("colorization_models", METHOD / "models" / "__init__.py")
        trainer = load("colorization_trainer",
                       METHOD / "train_pretrain_colorization.py")
        model = models.build_colorization_cnn(trainer.model_config(cfg["train"]))
    ...
    def _provider(self):
        return load("colorization_feature_provider",
    ...
    def test_it_returns_raw_512d_features_one_per_val_image(self):
        ...
        if not prov_path.is_file():
            self.skipTest("03_colorization provider not yet present")
        import numpy as np
        ...
        self.assertEqual(feats.shape[1], ENCODER_DIM,
                         "colorization trunk feature is 512-d")
