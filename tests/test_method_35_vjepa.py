...
# The shipped linear_eval config builds a vit_base V-JEPA encoder; its output is
# (N, 196, 768), so the mean-pooled feature is 768-d (measured, not assumed).
FEAT_DIM = 768
...
class TestFeatureProvider(Base):
    ...
    768-d V-JEPA backbone feature (the mean of the target encoder's output
    tokens, no CLS) -- raw, before the probe's normalise -- one row per val
    ...
    def _make_encoder(self, cfg: dict) -> Path:
        import torch
        models = load("vjepa_models", METHOD / "models" / "__init__.py")
        train = cfg["train"]
        encoder = models.build_vjepa_encoder(
            {k: train[k] for k in adapter.MODEL_KEYS}, torch.device("cpu"))
        state = adapter.extract_encoder(encoder.state_dict())
        encoder_pt = self.tmp / "encoder.pt"
        torch.save(state, encoder_pt)
        return encoder_pt

    def _provider(self):
        return load("vjepa_feature_provider", METHOD / "feature_provider.py")

    @needs_deps
    def test_it_returns_raw_768d_features_one_per_val_image(self):
        prov_path = METHOD / "feature_provider.py"
        if not prov_path.is_file():
            self.skipTest("35_vjepa provider not yet present")
        if not _submodule_present():
            self.skipTest("the jepa submodule is not checked out here")
        import numpy as np
        ...
        self.assertEqual(feats.shape[1], FEAT_DIM,
                         "V-JEPA token-mean feature is 768-d")
