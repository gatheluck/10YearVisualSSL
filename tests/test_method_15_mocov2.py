...
class TestFeatureProvider(Base):
    ...
    2048-d query ResNet-50 backbone feature -- raw, before the probe's
    ...
    def _make_encoder(self, cfg: dict) -> Path:
        import torch
        models = load("mocov2_models", METHOD / "models" / "__init__.py")
        trainer = load("mocov2_trainer",
                       METHOD / "train_pretrain_mocov2.py")
        model = models.build_mocov2_resnet(**trainer.model_config(cfg["train"]))
    ...
    def _provider(self):
        return load("mocov2_feature_provider",
    ...
    def test_it_returns_raw_2048d_features_one_per_val_image(self):
        ...
        if not prov_path.is_file():
            self.skipTest("15_mocov2 provider not yet present")
        import numpy as np
