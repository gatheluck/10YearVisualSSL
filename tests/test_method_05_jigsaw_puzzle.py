...
class TestFeatureProvider(Base):
    ...
    512-d shared CFN backbone feature -- raw, before the probe's normalise --
    ...
    def _make_encoder(self, cfg: dict) -> Path:
        import torch
        models = load("jigsaw_models", METHOD / "models" / "__init__.py")
        trainer = load("jigsaw_trainer", METHOD / "train_pretrain_jigsaw.py")
        model = models.build_alexnet_jigsaw_model(
    ...
    def _provider(self):
        return load("jigsaw_feature_provider", METHOD / "feature_provider.py")

    @needs_deps
    def test_it_returns_raw_512d_features_one_per_val_image(self):
        prov_path = METHOD / "feature_provider.py"
        if not prov_path.is_file():
            self.skipTest("05_jigsaw_puzzle provider not yet present")
        import numpy as np
        ...
        self.assertEqual(feats.shape[1], FEATURE_DIM,
                         "the shared CFN feature is 512-d")
...
class TestTheProbeEvalPreprocessing(Base):
    ...
    def evaluator(self):
        return load("jigsaw_eval", METHOD / "evaluate_linear_jigsaw.py")
