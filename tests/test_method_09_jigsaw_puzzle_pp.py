...
class TestTheDeviceIsResolved(Base):
    def trainer(self):
        return load("jigsaw_pp_trainer", METHOD / "train_pretrain_jigsaw_pp.py")
    ...
    def test_run_resolves_the_device(self):
        import ast
        src = (METHOD / "train_pretrain_jigsaw_pp.py").read_text()
...
class TestTheOriginalIsReferencedNotCopied(unittest.TestCase):
    def test_no_distributed_or_tensorboard_machinery_is_used(self):
        import ast
        tree = ast.parse((METHOD / "train_pretrain_jigsaw_pp.py").read_text())
        used = set()
...
# --- Step 2: unified ViT-B/16 (arch: vit), additive alongside the native VGG16
# pretrain and the knowledge_transfer stage. 9 processed tiles -> one 224 image.
...
VIT_MODEL_ARGS = {"num_classes": 4, "image_size": 32, "patch_size": 16,
                  ...
                  "hidden_dim": 8, "drop_rate": 0.0, "attn_drop_rate": 0.0}
VIT_TRAIN_TINY = {"arch": "vit", "num_permutations": 4, "tile_size": 10,
                  "tile_gap": 1, "image_size": 40, "puzzle_size": 32,
                  "grayscale_prob": 0.5, "max_occlusions": 1, "patch_size": 16,
                  "embed_dim": 16, "depth": 1, "num_heads": 2, "mlp_ratio": 4.0,
                  "hidden_dim": 8, "drop_rate": 0.0, "attn_drop_rate": 0.0,
                  "epochs": 2, "batch_size": 2, "num_workers": 0, "lr": 0.0006,
...
class TestVitConfigTranslation(Base):
    ...
    def test_a_bad_pretrain_arch_is_refused(self):
        with self.assertRaises(adapter.ConfigError) as e:
            adapter.to_run_config(self.vit_config(
                train={**VIT_TRAIN_TINY, "arch": "vgg16"}), out=self.out)
        self.assertIn("arch", str(e.exception))
...
class TestTheVitModel(unittest.TestCase):
    def _model(self):
        vj = load("vit_jigsaw_pp", METHOD / "models" / "vit_jigsaw_pp.py")
        return vj.build_vit_jigsaw_pp_model(**VIT_MODEL_ARGS)
...
class TestFeatureProvider(Base):
    ...
    1024-d VGG16 backbone feature -- raw, before the probe's normalise -- one
    ...
    def _make_encoder(self, cfg: dict) -> Path:
        import torch
        models = load("jigsaw_pp_models", METHOD / "models" / "__init__.py")
        trainer = load("jigsaw_pp_trainer",
                       METHOD / "train_pretrain_jigsaw_pp.py")
        model = models.build_vgg16_jigsaw_pp_model(
    ...
    def _provider(self):
        return load("jigsaw_pp_feature_provider",
    ...
    def test_it_returns_raw_1024d_features_one_per_val_image(self):
        ...
        if not prov_path.is_file():
            self.skipTest("09_jigsaw_puzzle_pp provider not yet present")
        import numpy as np
...
class TestTheProbeEvalPreprocessing(Base):
    ...
    the encoder's native tile size (this is a native-resolution backbone, so it
    keeps its own size, not 224). This method historically used a square resize
    with no centre crop; its val split (which the provider extracts) must now
    route through probe_transforms.basic5_eval_transform. The rule itself lives
    there; this confirms the wiring.
    """

    def evaluator(self):
        return load("jigsaw_pp_eval", METHOD / "evaluate_linear_jigsaw_pp.py")
