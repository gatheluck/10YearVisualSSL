...
# The shipped linear_eval.yaml builds a full NEPA ViT-B (embed_dim 768). The
# native probe pools the causal predictor output over positions ("avg"), one
# embed_dim vector per image; there is no `pool` key in the shipped config, so
# the provider takes the eval main's default "avg". This is the measured feature
# dimension the provider must return.
BACKBONE_DIM = 768
...
class TestFeatureProvider(Base):
    ...
    768-d NEPA feature (the pooled autoregressive predictor output, pool="avg"
    from the shipped config) -- raw, before the probe's normalise -- one row per
    val image, with honest meta.

    Unlike a trunk-only method, NEPA's eval passes the model *directly* to
    `extract_features` and pool is a positional argument; the provider mirrors
    that. The encoder.pt is built from the *shipped* linear_eval config's
    architecture (the provider reads that config): the model's own weights are
    wrapped under the adapter's `ema_model.` prefix and run back through the same
    `extract_encoder` filter the adapter writes with, so the plumbing is the
    real one. Random weights do not affect the shape-and-plumbing this proves.
    ...
    def _make_encoder(self, cfg: dict) -> Path:
        import torch
        models = load("nepa_models", METHOD / "models" / "__init__.py")
        trainer = load("nepa_trainer", METHOD / "train_pretrain_nepa.py")
        train = cfg["train"]
        model = models.build_nepa_model(
            **{k: train[k] for k in trainer.MODEL_ARGS})
        prefixed = {f"ema_model.{k}": v for k, v in model.state_dict().items()}
    ...
    def _provider(self):
        return load("nepa_feature_provider", METHOD / "feature_provider.py")

    @needs_deps
    def test_it_returns_raw_768d_features_one_per_val_image(self):
        prov_path = METHOD / "feature_provider.py"
        if not prov_path.is_file():
            self.skipTest("32_nepa provider not yet present")
        import numpy as np
        ...
        self.assertEqual(feats.shape[1], BACKBONE_DIM,
                         "NEPA pooled autoregressive feature is 768-d")
        ...
        self.assertEqual(meta["pooling"], "avg")
