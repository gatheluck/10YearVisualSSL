...
BACKBONE_DIM = 768  # the shipped linear_eval config is vit_base; get_backbone()
#                     returns the CLS token feature (embed_dim = 768) before the
#                     projector head.
...
class TestFeatureProvider(Base):
    ...
    768-d MoCo v3 base ViT CLS feature (before the projector head) -- raw,
    ...
    def _provider(self):
        return load("mocov3_feature_provider", METHOD / "feature_provider.py")

    @needs_deps
    def test_it_returns_raw_768d_features_one_per_val_image(self):
        prov_path = METHOD / "feature_provider.py"
        if not prov_path.is_file():
            self.skipTest("22_mocov3 provider not yet present")
        import numpy as np
        ...
        self.assertEqual(feats.shape[1], BACKBONE_DIM,
                         "MoCo v3 base ViT CLS feature is 768-d")
