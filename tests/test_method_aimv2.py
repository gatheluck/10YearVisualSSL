#!/usr/bin/env python3
"""Specification for aimv2 (AIMv2; Fini et al., 2024; arXiv:2411.14402).

A **pure eval-only** port: a single `linear_eval` stage and no pretraining at all,
the timm-sourced sibling of the download-and-probe methods (eva02 / 36_franca).
AIMv2's "Step 3 as-is" comparison freezes the official autoregressively pretrained
backbone (`aimv2_large_patch14_224.apple_pt`, a ViT built with RMSNorm, SwiGLU and
average pooling over patch tokens) and fits a linear probe on its pooled features,
because the from-scratch multimodal autoregressive pretraining (billions of
image-text pairs, many GPU-days) is the excluded step. The number is a genuine
learned representation, so it is comparable.

So this port ships no `encoder.pt` from training; `linear_eval` probes a frozen,
hash-pinned downloaded backbone. There is **no author submodule**: the model class
is timm's (a pinned pip dependency, `timm==1.0.28`), and the weights are a
...
smoke builds a **random tiny** AIMv2-style ViT (empty ckpt), so nothing is
downloaded. Licence: the AIMv2 weights are redistributed by timm under the Apple
Sample Code License (research-only); nothing is copied here.
"""
...
needs_deps = unittest.skipUnless(
    HAVE_DEPS, "aimv2 needs torch, torchvision and timm")
...
adapter = load("aimv2_adapter", METHOD / "adapter" / "__init__.py")

# A frozen backbone small enough to probe on a CPU: a random tiny AIMv2-style ViT
# (ckpt empty) -- 2 small blocks at 32px (patch 16 -> a 2x2 token grid), embed_dim
# 32, 2 heads. The shipped config pins the official AIMv2-Large/14 (embed 1024,
# 24 blocks) via bin/fetch-weights.py.
EVAL_TRAIN = {"name": "aimv2_large_patch14_224", "ckpt": "", "img_size": 32,
...
class Base(unittest.TestCase):
    def setUp(self) -> None:
        self.tmp = Path(tempfile.mkdtemp(prefix="aimv2-"))
...
class TestItIsAPureEvalOnlyPort(unittest.TestCase):
    ...
    def test_the_method_name_carries_no_number_and_no_step_word(self):
        self.assertEqual(adapter.METHOD, "aimv2")
...
class TestTheBackboneRepresentation(Base):
    def evaluator(self):
        return load("aimv2_eval", METHOD / "evaluate_linear_aimv2.py")
