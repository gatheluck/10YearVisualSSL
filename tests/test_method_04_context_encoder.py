...
FEATURE_DIM_ALEXNET = 4096  # the AlexNet bottleneck (model(x) -> (_, features))
...
class TestFeatureProvider(Base):
    ...
    4096-d AlexNet bottleneck feature -- raw, before the linear head -- one row
    per val image, with honest meta.

    The encoder.pt is built from the *shipped* linear_eval config's
    architecture (the native AlexNet, fixed channels=3), via the same
    ...
    def _make_encoder(self, cfg: dict) -> Path:
        import torch
        models = load("ce_models", METHOD / "models" / "__init__.py")
        model = models.create_model("alexnet", channels=3)
    ...
    def _provider(self):
        return load("ce_feature_provider", METHOD / "feature_provider.py")

    @needs_torch
    def test_it_returns_raw_4096d_features_one_per_val_image(self):
        prov_path = METHOD / "feature_provider.py"
        if not prov_path.is_file():
            self.skipTest("04_context_encoder provider not yet present")
        import numpy as np
        ...
        self.assertEqual(feats.shape[1], FEATURE_DIM_ALEXNET,
                         "AlexNet bottleneck is 4096-d")
        self.assertEqual(np.asarray(labels).shape[0], 4)
        self.assertEqual(meta["feat_dim"], FEATURE_DIM_ALEXNET)
    ...
    def test_the_driver_saves_it_under_a_per_method_directory(self):
        ...
        self.assertEqual(feats.shape, (4, FEATURE_DIM_ALEXNET))
