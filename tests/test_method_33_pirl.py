...
PRETRAIN_ONLY = {"epochs": 1, "batch_size": 2, "lr": 0.03, "momentum": 0.9,
              "weight_decay": 1.0e-4, "lr_milestones": [], "lr_gamma": 0.1,
              "warmup_epochs": 0}
TRAIN = {**MODEL, **DATA, **NCE, **LOSS, **MEMORY, **PRETRAIN_ONLY}
...
class TestTheDeviceIsResolved(Base):
    def trainer(self):
        return load("pirl_trainer", METHOD / "train_pretrain_pirl.py")
    ...
    def test_run_resolves_the_device(self):
        import ast
        src = (METHOD / "train_pretrain_pirl.py").read_text()
...
class TestTheOriginalIsReferencedNotCopied(unittest.TestCase):
    def test_no_distributed_or_tensorboard_machinery_is_used(self):
        import ast
        tree = ast.parse((METHOD / "train_pretrain_pirl.py").read_text())
        used = set()
...
# ResNet-50 PIRL pretrain. One ViT backbone; the jigsaw view reassembles the 9
# shuffled patches into an image and feeds the same ViT; memory-bank NCE +
# jigsaw NCE (weight 0.5). Tiny dims for a CPU smoke.
VIT_MODEL_ARGS = {"feature_dim": 8, "num_patches": 9, "image_size": 32,
                  "patch_size": 16, "embed_dim": 16, "depth": 1, "num_heads": 2,
                  "mlp_ratio": 4.0, "drop_rate": 0.0, "attn_drop_rate": 0.0}
VIT_TRAIN_TINY = {"arch": "vit", "feature_dim": 8, "num_patches": 9,
                  "image_size": 32, "jigsaw_resize": 48, "jigsaw_crop_size": 48,
                  "jigsaw_grid_size": 3, "jigsaw_patch_size": 16,
                  "num_workers": 0, "patch_size": 16, "embed_dim": 16,
                  "depth": 1, "num_heads": 2, "mlp_ratio": 4.0, "drop_rate": 0.0,
                  "attn_drop_rate": 0.0, "temperature": 0.07, "nce_momentum": 0.5,
                  "num_negatives": 4, "jigsaw_weight": 0.5,
                  "initialize_from_model": True, "epochs": 2, "batch_size": 2,
                  "lr": 6.0e-4, "weight_decay": 0.05, "warmup_epochs": 0,
                  "min_lr": 0.0, "clip_grad": 3.0, "save_at_epochs": [1, 2]}
...
class TestVitConfigTranslation(Base):
    ...
    def test_the_vit_step2_config_is_accepted(self):
        ...
        self.assertEqual(built["loss"]["jigsaw_weight"], 0.5)
...
class TestTheVitModel(unittest.TestCase):
    def _model(self):
        vm = load("vit_pirl", METHOD / "models" / "vit_pirl.py")
        return vm.build_vit_pirl(**VIT_MODEL_ARGS)

    def _img(self, torch, b=2):
        return torch.randn(b, 3, VIT_MODEL_ARGS["image_size"],
                           VIT_MODEL_ARGS["image_size"])

    def _patches(self, torch, b=2):
        p = VIT_MODEL_ARGS["patch_size"]
        return torch.randn(b, VIT_MODEL_ARGS["num_patches"], 3, p, p)

    @needs_timm
    def test_get_encoder_returns_the_cls_feature(self):
        import torch
        feats = self._model().get_encoder()(self._img(torch))
        self.assertEqual(tuple(feats.shape), (2, VIT_MODEL_ARGS["embed_dim"]))

    @needs_timm
    def test_forward_gives_image_and_jigsaw_embeddings(self):
        import torch
        img_f, jig_f = self._model()(self._img(torch), self._patches(torch))
        for f in (img_f, jig_f):
            self.assertEqual(tuple(f.shape), (2, VIT_MODEL_ARGS["feature_dim"]))
            norms = f.norm(dim=1)
            self.assertTrue(torch.allclose(norms, torch.ones_like(norms),
                                           atol=1e-4))

    @needs_timm
    def test_forward_without_patches_returns_only_the_image_embedding(self):
        import torch
        out = self._model()(self._img(torch))
        self.assertEqual(tuple(out.shape), (2, VIT_MODEL_ARGS["feature_dim"]))
    ...
    def test_load_encoder_round_trips_the_vit_backbone(self):
        ...
        cfg = {"train": {"arch": "vit", "feature_dim": 8, "num_patches": 9,
...
class TestFeatureProvider(Base):
    ...
    2048-d ResNet-50 trunk feature -- raw, before the probe's normalise -- one
    row per val image, with honest meta.

    The encoder.pt is built from the *shipped* linear_eval config's
    architecture (a plain ResNet-50 trunk; the provider reads that config), via
    the same `extract_encoder` filter the adapter writes with; random weights
    do not affect the shape-and-plumbing this proves. Modules load through
    `load` (`load_from`), which purges any other method's `adapter`/`models`
    first -- the whole suite runs many methods in one interpreter.
    """
    ...
    def _make_encoder(self, cfg: dict) -> Path:
        import torch
        models = load("pirl_models", METHOD / "models" / "__init__.py")
        # The ResNet-50 trunk is independent of feature_dim/num_patches (those
        # shape the excluded projection heads), so build defaults and keep
        # encoder.*, exactly as the adapter's load_encoder does.
        model = models.build_resnet_pirl()
    ...
    def _provider(self):
        return load("pirl_feature_provider", METHOD / "feature_provider.py")

    @needs_deps
    def test_it_returns_raw_2048d_features_one_per_val_image(self):
        prov_path = METHOD / "feature_provider.py"
        if not prov_path.is_file():
            self.skipTest("33_pirl provider not yet present")
        import numpy as np
        ...
        self.assertEqual(feats.shape[1], BACKBONE_DIM,
                         "ResNet-50 trunk feature is 2048-d")
