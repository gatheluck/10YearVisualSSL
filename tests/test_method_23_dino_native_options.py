...
class TestOptions(unittest.TestCase):
    ...
    def test_concatenates_last_layers_in_order_and_refuses_bad_count(self):
        spec = importlib.util.spec_from_file_location('native_dino_provider', METHOD / 'feature_provider.py')
        provider = importlib.util.module_from_spec(spec); spec.loader.exec_module(provider)
        class Tiny(torch.nn.Module):
            def __init__(self):
                super().__init__(); self.blocks = [None] * 4
            def get_intermediate_layers(self, x, n):
                return [torch.stack([torch.full((x.shape[0], 3), float(i)), torch.full((x.shape[0], 3), float(i + 100))], dim=1) for i in range(5-n, 5)]
        model = Tiny()
        wrapped = provider.native_backbone(model, 4)
        expected = torch.tensor([[1.,1.,1.,2.,2.,2.,3.,3.,3.,4.,4.,4.]])
        self.assertTrue(torch.equal(wrapped(torch.zeros(1,3)), expected))
        self.assertIs(provider.native_backbone(model, None), model)
        for n in (0, 5, True, '4'):
            with self.assertRaises(ValueError): provider.native_backbone(model, n)
    ...
    def test_options_reach_inference_and_defaults_are_preserved(self):
        ...
        imports = {'adapter': adapter, 'evaluate_linear_dino': evaluator}
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(dataset.transform.transforms[0].interpolation, T.InterpolationMode.BICUBIC)
...
class TestArtifact(unittest.TestCase):
    def test_exact_native_identity_and_protocol(self):
        ...
        self.assertEqual(r['sha256'], 'f43a051557c48d22e871bd185f34c7a140b4e7de1c5e1fdc19d352f2a80e6385')
        self.assertEqual(r['feature_options'], {'n_last_blocks': 4})
