...
class TestNativeProfile(unittest.TestCase):
    ...
    def test_profile_changes_actual_pixels_features_and_keeps_default(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            expected=normal.mean((2,3))+10
            self.assertEqual(configs[-1]['train']['arch'],'alexnet_cluster_cls')
...
class TestNativeState(unittest.TestCase):
    def test_native_knowledge_transfer_encoder_is_selected(self):
        import provider_support
        adapter=provider_support.import_sibling(METHOD,'adapter')
        tensor=object()
        self.assertEqual(adapter.extract_encoder({'features.0.weight':tensor,'classifier.1.weight':object()}),{'features.0.weight':tensor})
        with self.assertRaises(RuntimeError):adapter.extract_encoder({'features_extra.0.weight':tensor})

class TestArtifact(unittest.TestCase):
    def test_exact_checkpoint_and_protocol(self):
        r=json.loads((METHOD / "provenance.json").read_text())["step1_native_artifact"]
        expected={'sha256': 'e7c8721f2589e98db2138ec22d07b1e235b5d56dee534c25a951184e3a4b4653', 'state_key': 'state_dict', 'strip_prefix': '', 'availability': 'user_supplied', 'feature_options': {'feature_profile': 'imagenet_cluster_cls', 'config_overrides': {'train': {'arch': 'alexnet_cluster_cls', 'image_size': 224}}}}
