...
# ResNet-50/SGD MoCo v1 pretrain. Momentum encoder + FIFO queue + InfoNCE, the
# CLS token through a single Linear projection; tiny dims for a CPU smoke. The
# queue size must divide the batch (K=4, batch=2).
    ...
    def test_native_knob_does_not_leak_into_the_vit_path(self):
        with self.assertRaises(adapter.ConfigError) as e:
            adapter.to_run_config(
                self.vit_config(train={**VIT_TRAIN_TINY, "lr_decay_rate": 0.1}),
                out=self.out)
        self.assertIn("lr_decay_rate", str(e.exception))


class TestTheVitModel(unittest.TestCase):
    def _model(self):
        vm = load("vit_moco", METHOD / "models" / "vit_moco.py")
        return vm.build_moco_vit(**VIT_MODEL_ARGS, **VIT_QUEUE)
...
class TestFeatureProvider(Base):
    ...
    def _make_encoder(self, cfg: dict) -> Path:
        import torch
        models = load("mocov1_models", METHOD / "models" / "__init__.py")
        trainer = load("mocov1_trainer",
                       METHOD / "train_pretrain_mocov1.py")
        model = models.build_moco_resnet(**trainer.model_config(cfg["train"]))
    ...
    def _provider(self):
        return load("mocov1_feature_provider",
    ...
    def test_it_returns_raw_2048d_features_one_per_val_image(self):
        ...
        if not prov_path.is_file():
            self.skipTest("13_mocov1 provider not yet present")
        import numpy as np
...
class TestTheProbeTrainAugmentation(Base):
    ...
    def evaluator(self):
        return load("mocov1_eval", METHOD / "evaluate_linear_mocov1.py")
    ...
    def test_run_builds_the_train_loader_with_augmentation(self):
        _probe_aug.assert_run_wires_train_split(
            self, METHOD / "evaluate_linear_mocov1.py")
