...
# unified schedule values (epochs 300, batch 1024, lr 6e-4). Same MIM objective,
# masking and dVAE tokenizer. The smoke keeps the tiny dims + random tokenizer.
UNIFIED_TRAIN = {"recipe": "unified", **MODEL, **TOKENIZER, "num_workers": 0,
                 "num_masking_patches": NUM_MASK, "min_num_patches": MIN_MASK,
                 "epochs": 2, "batch_size": 2, "lr": 6.0e-4, "beta1": 0.9,
                 "beta2": 0.999, "eps": 1.0e-8, "weight_decay": 0.05,
...
BACKBONE_DIM = 768  # BEiT-Base embed_dim; the mean of the patch tokens (CLS
                    # excluded) is one embed_dim vector per image (measured)
...
class TestFeatureProvider(Base):
    ...
    768-d BEiT backbone feature (the mean of the patch tokens with the CLS
    token excluded) -- raw, before the probe's normalise -- one row per val
    image, with honest meta.
    ...
    def _make_encoder(self, cfg: dict) -> Path:
        import torch
        models = load("beit_models", METHOD / "models" / "__init__.py")
        trainer = load("beit_trainer", METHOD / "train_pretrain_beit.py")
        train = cfg["train"]
        model = models.build_beit(**{k: train[k] for k in trainer.MODEL_ARGS})
    ...
    def _provider(self):
        return load("beit_feature_provider", METHOD / "feature_provider.py")

    @needs_deps
    def test_it_returns_raw_768d_features_one_per_val_image(self):
        prov_path = METHOD / "feature_provider.py"
        if not prov_path.is_file():
            self.skipTest("24_beit provider not yet present")
        import numpy as np
        ...
        self.assertEqual(feats.shape[1], BACKBONE_DIM,
                         "BEiT patch-token-mean feature is 768-d")
