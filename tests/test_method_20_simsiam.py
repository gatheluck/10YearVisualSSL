...
BACKBONE_DIM = 2048  # ResNet-50 pooled feature (measured: encoder out is
#                      (N, 2048, 1, 1), flattened to (N, 2048) by the eval main)
...
class TestFeatureProvider(Base):
    ...
    own encoder loader and the official evaluation's `val` pipeline, so the
    check is that it returns the 2048-d ResNet-50 backbone feature -- raw,
    before the probe's head -- one row per val image, with honest meta.
    ...
    def _make_encoder(self, cfg: dict) -> Path:
        ...
        model = models.build_simsiam_resnet(dim=int(cfg["train"]["dim"]),
                                            pred_dim=int(cfg["train"]["pred_dim"]))
    ...
    def _provider(self):
        return load("simsiam_feature_provider", METHOD / "feature_provider.py")

    @needs_torch
    def test_it_returns_raw_2048d_features_one_per_val_image(self):
        prov_path = METHOD / "feature_provider.py"
        if not prov_path.is_file():
            self.skipTest("20_simsiam provider not yet present")
        import numpy as np
        data_root = tiny_classified(self.tmp / "data")  # 2 classes x 2 = 4 val
    ...
    def test_the_driver_saves_it_under_a_per_method_directory(self):
        ...
        self.assertEqual(feats.shape, (4, BACKBONE_DIM))
