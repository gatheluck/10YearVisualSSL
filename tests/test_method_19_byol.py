...
class TestTheDeviceIsResolved(Base):
    def trainer(self):
        return load("byol_trainer", METHOD / "train_pretrain_byol.py")
    ...
    def test_run_resolves_the_device(self):
        import ast
        src = (METHOD / "train_pretrain_byol.py").read_text()
...
class TestTheOriginalIsReferencedNotCopied(unittest.TestCase):
    def test_no_distributed_or_tensorboard_machinery_is_used(self):
        import ast
        tree = ast.parse((METHOD / "train_pretrain_byol.py").read_text())
        used = set()
...
# ResNet-50/LARS BYOL pretrain. Online/target ViT encoders + projector +
# predictor, EMA target, symmetric negative-cosine; tiny dims for a CPU smoke
# (batch_size must be >1 for the projector/predictor BatchNorm1d).
VIT_MODEL_ARGS = {"encoder_dim": 16, "proj_hidden_dim": 32, "proj_output_dim": 8,
                  "pred_hidden_dim": 32, "pred_output_dim": 8, "image_size": 32,
                  "patch_size": 16, "depth": 1, "num_heads": 2, "mlp_ratio": 4.0,
                  "drop_rate": 0.0, "attn_drop_rate": 0.0}
VIT_TRAIN_TINY = {"arch": "vit", "encoder_dim": 16, "proj_hidden_dim": 32,
                  "proj_output_dim": 8, "pred_hidden_dim": 32,
                  "pred_output_dim": 8, "image_size": 32, "patch_size": 16,
                  "depth": 1, "num_heads": 2, "mlp_ratio": 4.0, "drop_rate": 0.0,
                  "attn_drop_rate": 0.0, "epochs": 2, "batch_size": 2,
                  "num_workers": 0, "learning_rate": 6.0e-4,
                  "lr_scale_by_batch": False, "lr_scale_base": 256,
                  "weight_decay": 0.05, "warmup_epochs": 0, "min_lr": 1.0e-6,
                  "ema_tau_base": 0.996, "ema_tau_final": 1.0, "clip_grad": 1.0,
                  "save_at_epochs": [1, 2]}
...
class TestVitConfigTranslation(Base):
    ...
    def test_the_vit_step2_config_is_accepted(self):
        ...
        self.assertEqual(built["model"]["encoder_dim"], 16)
    ...
    def test_native_lars_knob_does_not_leak_into_the_vit_path(self):
        with self.assertRaises(adapter.ConfigError) as e:
            adapter.to_run_config(
                self.vit_config(train={**VIT_TRAIN_TINY,
                                       "trust_coefficient": 0.001}),
                out=self.out)
        self.assertIn("trust_coefficient", str(e.exception))


class TestTheVitModel(unittest.TestCase):
    def _model(self):
        vm = load("vit_byol", METHOD / "models" / "vit_byol.py")
        return vm.build_byol_vit(**VIT_MODEL_ARGS)
    ...
    @needs_timm
    def test_encode_returns_the_cls_feature(self):
        import torch
        feats = self._model().encode(self._batch(torch))
        self.assertEqual(tuple(feats.shape), (2, VIT_MODEL_ARGS["encoder_dim"]))

    @needs_timm
    def test_forward_returns_online_predictions_and_frozen_target(self):
        import torch
        model = self._model()
        model.train()
        p1, p2, z1, z2 = model(self._batch(torch), self._batch(torch))
        for t in (p1, p2, z1, z2):
            self.assertEqual(tuple(t.shape), (2, VIT_MODEL_ARGS["proj_output_dim"]))
        self.assertTrue(p1.requires_grad)
        self.assertFalse(z1.requires_grad, "target projection must be detached")
    ...
    @needs_timm
    def test_encoder_pt_holds_only_the_online_backbone(self):
        got = adapter.extract_encoder(self._model().state_dict())
        self.assertTrue(got)
        self.assertTrue(all(k.startswith("online_encoder.") for k in got))
        self.assertFalse([k for k in got if k.startswith("online_projector")])
        self.assertFalse([k for k in got if k.startswith("predictor")])
        self.assertFalse([k for k in got if k.startswith("target_")])

    @needs_timm
    def test_load_encoder_round_trips_the_online_backbone(self):
        import torch
    ...
    def _eval_cfg(self, encoder) -> dict:
        return {"stage": "linear_eval", "seed": 0,
                ...
                "train": {"arch": "vit", "image_size": 32, "patch_size": 16,
                          "encoder_dim": 16, "depth": 1, "num_heads": 2,
...
class TestFeatureProvider(Base):
    ...
    2048-d online ResNet-50 backbone feature -- raw, before the probe's
    normalise -- one row per val image, with honest meta. Unlike a method with
    a `get_encoder()`, BYOL hands the model to `extract_features` directly,
    which reads it through `model.encode()`.
    ...
    def _make_encoder(self, cfg: dict) -> Path:
        import torch
        models = load("byol_models", METHOD / "models" / "__init__.py")
        model = models.build_byol_resnet50({})
    ...
    def _provider(self):
        return load("byol_feature_provider",
    ...
    def test_it_returns_raw_2048d_features_one_per_val_image(self):
        ...
        if not prov_path.is_file():
            self.skipTest("19_byol provider not yet present")
        import numpy as np
