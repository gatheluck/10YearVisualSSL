...
class TestNativeProfile(unittest.TestCase):
    ...
    def test_profile_reaches_model_inference_and_preserves_defaults(self):
        ...
        options = {'config_overrides': {'train': {'latent_dim': 50, 'img_size': 224}}, 'resize_short_side': 256}
        with tempfile.TemporaryDirectory() as tmp:
            encoder = Path(tmp) / 'encoder.pt'; torch.save({}, encoder)
            def run():
            ...
            self.assertEqual(configs[-1]['train']['latent_dim'], 50)
            self.assertEqual(configs[-1]['train']['img_size'], 224)
            self.assertEqual(dataset.transform.transforms[0].size, 256)

class TestArtifact(unittest.TestCase):
    def test_exact_checkpoint_and_protocol(self):
        r = json.loads((METHOD / "provenance.json").read_text())["step1_native_artifact"]
        self.assertEqual(r['sha256'], 'c3b736c76a6c77a109d7151b03505da34343748a6ae85d56ac3510500d88cd00')
        self.assertEqual(r['availability'], 'user_supplied')
        self.assertEqual(r['state_key'], 'model_state_dict')
        self.assertEqual(r['strip_prefix'], '')
        self.assertEqual(r['feature_options'], {'config_overrides': {'train': {'latent_dim': 50, 'img_size': 224}}, 'resize_short_side': 256})
        self.assertEqual(r['checkpoint_epoch_stored'], 300)
