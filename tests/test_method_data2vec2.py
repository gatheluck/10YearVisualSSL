#!/usr/bin/env python3
"""Specification for data2vec2 (data2vec 2.0; Baevski et al., 2023; arXiv:2212.07525).

A **pure eval-only** port: a single `linear_eval` stage and no pretraining at all,
the HuggingFace-sourced sibling of the download-and-probe methods (eva02 / aimv2 /
36_franca). data2vec 2.0's "Step 3 as-is" comparison freezes the official
pretrained vision backbone (`facebook/data2vec-vision-base`, a BEiT-architecture
ViT) and fits a linear probe on its **CLS token**, because the from-scratch
self-distillation pretraining (IN-1k/IN-22k, many GPU-days) is the excluded step.
The number is a genuine SSL representation, so it is comparable.

The capture's ARSSL harness loads this backbone with the `transformers` library
(`Data2VecVisionModel`, CLS token) -- so, unlike eva02/aimv2 (timm) or beitv2 (a
microsoft/unilm submodule), the model class here is `transformers`', a pinned pip
dependency new to the fleet, not a git submodule. A real run loads the official
checkpoint (a hash-pinned download, passed as `ckpt`) into the named architecture;
the hermetic smoke leaves `ckpt` empty and builds a **random tiny**
Data2VecVisionModel from the config's architecture keys, so nothing is downloaded
and the pipeline runs on a CPU. Licence: the data2vec-vision weights are Apache-2.0
(Meta AI, via HuggingFace); nothing is copied here.
"""
...
needs_deps = unittest.skipUnless(
    HAVE_DEPS, "data2vec2 needs torch, torchvision and transformers")
...
adapter = load("data2vec2_adapter", METHOD / "adapter" / "__init__.py")

# A frozen backbone small enough to probe on a CPU: a random tiny data2vec-vision
# (ckpt empty) -- 2 small blocks at 32px (patch 16 -> a 2x2 token grid), embed_dim
# 32, 2 heads. The shipped config pins the official data2vec-vision-base (embed
# 768, 12 blocks) via bin/fetch-weights.py.
EVAL_TRAIN = {"name": "facebook/data2vec-vision-base", "ckpt": "", "img_size": 32,
...
class Base(unittest.TestCase):
    def setUp(self) -> None:
        self.tmp = Path(tempfile.mkdtemp(prefix="data2vec2-"))
...
class TestItIsAPureEvalOnlyPort(unittest.TestCase):
    ...
    def test_the_method_name_carries_no_number_and_no_step_word(self):
        self.assertEqual(adapter.METHOD, "data2vec2")
...
class TestTheBackboneRepresentation(Base):
    def evaluator(self):
        return load("data2vec2_eval", METHOD / "evaluate_linear_data2vec2.py")
...
# data2vec-vision-base embed_dim: the CLS token of last_hidden_state is that
# wide (measured from the shipped config's embed_dim, which the provider reads).
BACKBONE_DIM = 768
...
class TestFeatureProvider(Base):
    ...
    own eval pipeline, so the check is that it returns the 768-d data2vec-vision
    backbone feature (the CLS token of last_hidden_state) -- raw, before the
    probe's normalise -- one row per val image, with honest meta.

    This port is eval-only: there is no `adapter.load_encoder`. The encoder.pt
    is the state_dict of a Data2VecVisionModel built from the *shipped*
    linear_eval config's architecture (the provider reads that config and loads
    the checkpoint through `train["ckpt"]`, exactly as a real run does with the
    sha256-pinned download). Random weights do not affect the shape-and-plumbing
    this proves. Modules load through `load` (`load_from`), which purges any
    other method's modules first -- the whole suite runs many methods in one
    interpreter.
    """
    ...
    def _make_encoder(self, cfg: dict) -> Path:
        import torch
        ev = load("data2vec2_eval", METHOD / "evaluate_linear_data2vec2.py")
        train = dict(cfg["train"])
        train["ckpt"] = ""  # build a random model from the shipped architecture
        model = ev.build_model(train, torch.device("cpu"))
        encoder_pt = self.tmp / "encoder.pt"
        torch.save(model.state_dict(), encoder_pt)
        return encoder_pt

    def _provider(self):
        return load("data2vec2_feature_provider", METHOD / "feature_provider.py")

    @needs_deps
    def test_it_returns_raw_768d_features_one_per_val_image(self):
        prov_path = METHOD / "feature_provider.py"
        if not prov_path.is_file():
            self.skipTest("data2vec2 provider not yet present")
        import numpy as np
        ...
        self.assertEqual(feats.shape[1], BACKBONE_DIM,
                         "data2vec-vision CLS feature is 768-d")
...
class TestTheProbeTrainAugmentation(Base):
    ...
    This method's val transform is Resize (shorter side) + CenterCrop (rule b),
    so the val control asserts the centre crop is kept (must_include="CenterCrop").
    """
    ...
    def test_run_builds_the_train_loader_with_augmentation(self):
        _probe_aug.assert_run_wires_train_split(
            self, METHOD / "evaluate_linear_data2vec2.py")


class TestTheProbeEvalPreprocessing(Base):
    """BASIC5_FAIR_v1 rule `b`: the eval preprocessing is Resize (shorter side)
    256 + CenterCrop 224 -- a deterministic centre crop that preserves aspect
    ratio. This method historically used a bicubic *square* resize with no centre
    crop; its val split (which the provider extracts) must now route through
