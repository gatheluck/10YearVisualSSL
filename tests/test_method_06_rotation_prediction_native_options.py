    ...
    def test_profile_changes_actual_pixels_features_and_keeps_default(self):
        ...
        class Encoder(torch.nn.Module):
            def __init__(self):
                ...
                class PoolSentinel(torch.nn.Module):
                    def forward(self, x): return x * 2
                self.features[-1] = PoolSentinel()
...
class TestNativeState(unittest.TestCase):
    def test_exact_block_names_preserve_tensors_and_reject_decoys(self):
        import provider_support
        adapter=provider_support.import_sibling(METHOD,'adapter')
        native={'_feature_blocks.0.0.weight':object(), '_feature_blocks.2.1.running_mean':object(),
                '_feature_blocks.6.0.bias':object(), '_feature_blocks.8.1.weight':object(),
                '_feature_blocks.9.0.weight':object()}
        expected={'encoder.features.0.weight':native['_feature_blocks.0.0.weight'],
                  'encoder.features.5.running_mean':native['_feature_blocks.2.1.running_mean'],
                  'encoder.features.14.bias':native['_feature_blocks.6.0.bias'],
                  'encoder.fc_block.1.weight':native['_feature_blocks.8.1.weight']}
        self.assertEqual(adapter.extract_encoder(native),expected)
        for key in ['_feature_blocks_extra.0.0.weight','_feature_blocks.10.0.weight']:
            with self.subTest(key=key), self.assertRaises((ValueError,RuntimeError)):
                adapter.extract_encoder({key:object()})
        with self.assertRaises((ValueError,RuntimeError)):
            adapter.extract_encoder({**native,'encoder.features.0.weight':object()})

class TestArtifact(unittest.TestCase):
    def test_exact_checkpoint_and_protocol(self):
        r=json.loads((METHOD / "provenance.json").read_text())["step1_native_artifact"]
        expected={'sha256': 'fb432e898219813abad57859ab13b158a8dc1f593c753d94150271447df6f04b', 'state_key': 'state_dict', 'strip_prefix': '', 'availability': 'user_supplied', 'feature_options': {'feature_profile': 'imagenet_conv5'}}
