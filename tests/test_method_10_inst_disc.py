...
class TestFeatureProvider(Base):
    ...
    def _make_encoder(self, cfg: dict) -> Path:
        import torch
        models = load("inst_disc_models", METHOD / "models" / "__init__.py")
        trainer = load("inst_disc_trainer",
                       METHOD / "train_pretrain_instdisc.py")
        model = models.build_resnet_instdisc(
    ...
    def _provider(self):
        return load("inst_disc_feature_provider",
    ...
    def test_it_returns_raw_2048d_features_one_per_val_image(self):
        ...
        if not prov_path.is_file():
            self.skipTest("10_inst_disc provider not yet present")
        import numpy as np
...
class TestTheProbeEvalPreprocessing(Base):
    ...
    ratio. This method historically used a square resize with no centre crop; its
    ...
    def evaluator(self):
        return load("inst_disc_eval", METHOD / "evaluate_linear_instdisc.py")
