...
class TestFeatureProvider(Base):
    ...
    def _make_encoder(self, cfg: dict) -> Path:
        import torch
        models = load("simclr_models", METHOD / "models" / "__init__.py")
        trainer = load("simclr_trainer",
                       METHOD / "train_pretrain_simclr.py")
        model = models.build_resnet_simclr(**trainer.model_config(cfg["train"]))
    ...
    def _provider(self):
        return load("simclr_feature_provider",
