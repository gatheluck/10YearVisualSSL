...
STEP1_ONLY = {"epochs": 1, "batch_size": 2, "num_workers": 0, "lr": 0.5,
              "momentum": 0.9, "weight_decay": 1.0e-6, "eta": 0.001,
              "warmup_epochs": 0}
TRAIN = {**MODEL, **LOSS, **DATA, **STEP1_ONLY}
...
class TestTheDeviceIsResolved(Base):
    def trainer(self):
        return load("simclrv2_trainer", METHOD / "train_step1_simclrv2.py")
    ...
    def test_run_resolves_the_device(self):
        import ast
        src = (METHOD / "train_step1_simclrv2.py").read_text()
...
class TestTheOriginalIsReferencedNotCopied(unittest.TestCase):
    def test_no_distributed_or_tensorboard_machinery_is_used(self):
        import ast
        tree = ast.parse((METHOD / "train_step1_simclrv2.py").read_text())
        used = set()
