#!/usr/bin/env python3
"""Specification for 16_simclrv2 (Chen et al., 2020; arXiv:2006.10029).

SimCLR v2, the **ResNet-50** path. Same contrastive objective as SimCLR v1: two
independently-augmented views feed a shared ResNet-50 and a projection head; the
**NT-Xent** loss (temperature 0.1) pulls the two views of an image together and
pushes every other image in the batch apart, optimised with **LARS**. The v2
change is a **3-layer MLP projection head** (v1 used 2 layers), with an optional
`width_multiplier`. `encoder.pt` is the ResNet-50 backbone; `linear_eval` probes
it (2048-d).
...
METHOD = ROOT / "methods" / "16_simclrv2"
BIN = ROOT / "bin"
...
needs_deps = unittest.skipUnless(
    HAVE_DEPS, "16_simclrv2 needs torch, numpy, torchvision")
...
adapter = load("simclrv2_adapter", METHOD / "adapter" / "__init__.py")

# Small enough to train a step on a CPU: a narrow projection, a 32px input
# (ResNet-50 downsamples to 1x1, still valid), width_multiplier 1. The paper's
# 224px / out_dim 128 / LARS lr 4.8 / 800 epochs live in the shipped config.
MODEL = {"out_dim": 32, "img_size": 32, "width_multiplier": 1}
...
class Base(unittest.TestCase):
    def setUp(self) -> None:
        self.tmp = Path(tempfile.mkdtemp(prefix="simclrv2-"))
...
class TestTheModel(unittest.TestCase):
    def models(self):
        return load("simclrv2_models", METHOD / "models" / "__init__.py")
    ...
    def test_forward_projects_and_l2_normalises(self):
        import torch
        m = self.models()
        model = m.build_resnet_simclrv2(out_dim=MODEL["out_dim"])
        model.eval()
    ...
    @needs_deps
    def test_the_projection_head_is_a_three_layer_mlp(self):
        # SimCLR v2's headline change over v1: a 3-layer MLP head (three Linear,
        # two ReLU), not v1's two-layer head. The extra layer + nonlinearity is
        ...
        relus = [x for x in proj.modules() if isinstance(x, nn.ReLU)]
        self.assertEqual(len(linears), 3, "SimCLR v2 uses a 3-layer MLP head")
        self.assertEqual(len(relus), 2,
                         "a 3-layer MLP has two nonlinearities (v1 has one)")
        self.assertEqual(linears[-1].out_features, MODEL["out_dim"])

    @needs_deps
    def test_the_encoder_returns_the_backbone_feature(self):
        import torch
        m = self.models()
        enc = m.build_resnet_simclrv2(out_dim=MODEL["out_dim"]).get_encoder()
        enc.eval()
...
class TestTheLoss(unittest.TestCase):
    def loss_mod(self):
        return load("simclrv2_loss", METHOD / "loss" / "__init__.py")
    ...
    def test_it_prefers_the_matched_positive(self):
        ...
        aligned = crit(z, z.clone())
        shuffled = crit(z, z.flip(0))
    ...
    def test_the_self_term_is_excluded_not_merely_set_low(self):
        ...
        # loss depends on the diagonal being *excluded* (-inf), not left in. Two
        # orthonormal directions, both views equal: each row competes its
        # positive (sim 1/tau=0.1) against two zero negatives, self excluded, so
        # every row's loss is -log(e^0.1 / (2*e^0 + e^0.1)) = 1.03307 (fp32).
        import torch
...
class TestTheOptimizer(unittest.TestCase):
    def optim_mod(self):
        return load("simclrv2_optim", METHOD / "optim" / "__init__.py")
...
class TestTheDataset(Base):
    def dataset_mod(self):
        return load("simclrv2_data", METHOD / "data" / "__init__.py")

    @needs_deps
    def test_an_item_is_two_views_and_a_label(self):
        import torch
        tiny_imagefolder(self.tmp / "data")
        ds = self.dataset_mod().SimCLRv2Dataset(
...
class TestExtractingTheEncoder(unittest.TestCase):
    def test_only_the_backbone_comes_out(self):
        got = adapter.extract_encoder({
            ...
            "projector.0.weight": 3, "projector.4.weight": 4})
...
class TestConfigTranslation(Base):
    def test_step1_reaches_the_run_config(self):
        ...
        self.assertEqual(built["model"]["width_multiplier"], 1)
...
class TestTheDeviceIsResolved(Base):
    def trainer(self):
        return load("simclrv2_trainer", METHOD / "train_pretrain_simclrv2.py")
    ...
    def test_run_resolves_the_device(self):
        import ast
        src = (METHOD / "train_pretrain_simclrv2.py").read_text()
...
class TestTheOriginalIsReferencedNotCopied(unittest.TestCase):
    def test_no_distributed_or_tensorboard_machinery_is_used(self):
        import ast
        tree = ast.parse((METHOD / "train_pretrain_simclrv2.py").read_text())
        used = set()
...
# ResNet-50/LARS pretrain. 3-layer projector, NT-Xent temperature 0.1.
...
VIT_TRAIN_TINY = {"arch": "vit", "out_dim": 8, "img_size": 32, "patch_size": 16,
                  "embed_dim": 16, "depth": 1, "num_heads": 2, "mlp_ratio": 4.0,
                  "drop_rate": 0.0, "attn_drop_rate": 0.0, "temperature": 0.1,
    ...
    def test_native_knob_does_not_leak_into_the_vit_path(self):
        with self.assertRaises(adapter.ConfigError) as e:
            adapter.to_run_config(
                self.vit_config(train={**VIT_TRAIN_TINY, "width_multiplier": 1}),
                out=self.out)
        self.assertIn("width_multiplier", str(e.exception))


class TestTheVitModel(unittest.TestCase):
    def _model(self):
        vs = load("vit_simclrv2", METHOD / "models" / "vit_simclrv2.py")
        return vs.build_vit_simclrv2(**VIT_MODEL_ARGS)
...
class TestFeatureProvider(Base):
    ...
    2048-d ResNet-50 backbone feature -- raw, before the probe's normalise --
    ...
    def _make_encoder(self, cfg: dict) -> Path:
        import torch
        models = load("simclrv2_models", METHOD / "models" / "__init__.py")
        trainer = load("simclrv2_trainer",
                       METHOD / "train_pretrain_simclrv2.py")
        model = models.build_resnet_simclrv2(**trainer.model_config(cfg["train"]))
    ...
    def _provider(self):
        return load("simclrv2_feature_provider",
    ...
    def test_it_returns_raw_2048d_features_one_per_val_image(self):
        ...
        if not prov_path.is_file():
            self.skipTest("16_simclrv2 provider not yet present")
        import numpy as np
