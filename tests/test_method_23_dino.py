...
# ViT-S/16 paper recipe. The SAME DINO objective/head/multi-crop, but arch:
# vit_base and the unified recipe (fixed weight decay + milestone checkpoints).
# Selected by an explicit `recipe` key (arch already means model size here).
# vit_base at a 32px input keeps the CPU smoke small.
UNIFIED_MODEL = {"arch": "vit_base", "img_size": IMG}
UNIFIED_ONLY = {"epochs": 2, "batch_size": 2, "num_workers": 0,
                "drop_path_rate": 0.0, "lr": 6.0e-4, "min_lr": 1.0e-6,
                "warmup_epochs": 0, "weight_decay": 0.05, "clip_grad": 3.0,
                "freeze_last_layer": 0, "save_at_epochs": [1, 2]}
UNIFIED_TRAIN = {"recipe": "unified", **UNIFIED_MODEL, **DINO, **UNIFIED_ONLY}
UNIFIED_EVAL_TRAIN = {"arch": "vit_base", "img_size": IMG, "epochs": 2,
                      "batch_size": 2, "num_workers": 0, "lr": 0.1,
                      "momentum": 0.9, "weight_decay": 0.0}
EMBED_DIM_VIT_B = 768  # vit_base CLS feature
...
class TestUnifiedConfigTranslation(Base):
    ...
    def test_the_unified_step2_config_is_accepted(self):
        ...
        self.assertNotIn("weight_decay_start", built["training"])
    ...
    def test_a_native_knob_does_not_leak_into_the_unified_path(self):
        for key in ("weight_decay_start", "weight_decay_end"):
            with self.subTest(key=key):
                with self.assertRaises(adapter.ConfigError) as e:
                    adapter.to_run_config(
                        self.unified_config(train={**UNIFIED_TRAIN, key: 0.1}),
                        self.out)
                self.assertIn(key, str(e.exception))

    def test_a_unified_knob_does_not_leak_into_the_native_path(self):
        for key in ("weight_decay", "save_at_epochs"):
            with self.subTest(key=key):
                with self.assertRaises(adapter.ConfigError) as e:
...
class TestFeatureProvider(Base):
    ...
    384-d DINO teacher ViT feature (the CLS token after the final norm) -- raw,
    before the probe's normalise -- one row per val image, with honest meta.

    The encoder.pt is built from the *shipped* linear_eval config's
    architecture (the provider reads that config), then run through the same
    `extract_encoder` filter the adapter writes with (the teacher.backbone.
    prefix stripped); random weights do not affect the shape-and-plumbing this
    proves. The eval main passes the model directly as the backbone -- there is
    no get_encoder -- and the provider mirrors that. Modules load through `load`
    ...
    def _make_encoder(self, cfg: dict) -> Path:
        import torch
        models = load("dino_models", METHOD / "models" / "__init__.py")
        train = cfg["train"]
        model = models.build_vit(str(train["arch"]),
                                 img_size=int(train["img_size"]))
        # encoder.pt is the teacher backbone; feed extract_encoder the prefixed
        # keys it filters on in a real run, then keep what it hands back.
        prefixed = {(adapter.ENCODER_PREFIX + k): v
                    for k, v in model.state_dict().items()}
        state = adapter.extract_encoder(prefixed)
        encoder_pt = self.tmp / "encoder.pt"
        torch.save(state, encoder_pt)
        return encoder_pt

    def _provider(self):
        return load("dino_feature_provider", METHOD / "feature_provider.py")

    @needs_deps
    def test_it_returns_raw_384d_features_one_per_val_image(self):
        prov_path = METHOD / "feature_provider.py"
        if not prov_path.is_file():
            self.skipTest("23_dino provider not yet present")
        import numpy as np
        ...
        self.assertEqual(feats.shape[1], EMBED_DIM,
                         "DINO teacher CLS feature is 384-d")
...
class TestTheProbeTrainAugmentation(Base):
    ...
    def evaluator(self):
        return load("dino_eval", METHOD / "evaluate_linear_dino.py")
    ...
    def test_run_builds_the_train_loader_with_augmentation(self):
        _probe_aug.assert_run_wires_train_split(
            self, METHOD / "evaluate_linear_dino.py")
