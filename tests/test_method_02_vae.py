...
class TestFeatureProvider(Base):
    ...
    VAE's latent mean `mu` -- raw, before the probe's linear layer -- one row
    per image in the split, with honest meta.

    The VAE has no `get_encoder`; the representation is `mu`
    (`VAE_CNN.get_features`), a `latent_dim`-d vector. `encoder.pt` is built
    ...
    random weights do not affect the shape-and-plumbing this proves. The dataset
    is the shipped MNIST (dataset-agnostic loader, MNIST branch), whose val
    split (`train=False`) holds the fabricated t10k images. Modules load through
    `load` (`load_from`), which purges any other method's `adapter`/`models`
    first -- the whole suite runs many methods in one interpreter.
    """

    LATENT_DIM = 20  # the shipped linear_eval.yaml's latent_dim = feat_dim (mu)
    VAL_IMAGES = 8    # tiny_mnist writes 8 t10k (val) images
    ...
    def _make_encoder(self, cfg: dict) -> Path:
        import torch
        vc = load("vae_feat_cnn", METHOD / "models" / "vae_cnn.py")
        train = cfg["train"]
        model = vc.VAE_CNN(latent_dim=int(train["latent_dim"]),
    ...
    def _provider(self):
        return load("vae_feature_provider", METHOD / "feature_provider.py")

    @needs_torch
    def test_it_returns_raw_mu_features_one_per_val_image(self):
        prov_path = METHOD / "feature_provider.py"
        if not prov_path.is_file():
            self.skipTest("02_vae provider not yet present")
        import numpy as np
        tiny_mnist(self.tmp / "data")
        ...
        self.assertEqual(feats.shape[0], self.VAL_IMAGES,
                         "8 val (t10k) images expected")
        self.assertEqual(feats.shape[1], self.LATENT_DIM,
                         "the VAE feature is the latent mean mu (latent_dim)")
        self.assertEqual(np.asarray(labels).shape[0], self.VAL_IMAGES)
        self.assertEqual(meta["feat_dim"], self.LATENT_DIM)
    ...
    def test_the_driver_saves_it_under_a_per_method_directory(self):
        ...
        self.assertEqual(labels.shape[0], self.VAL_IMAGES)
    ...
    def test_the_isolated_driver_run_extracts_this_method_end_to_end(self):
        """The whole driver, real subprocess, real provider. It runs a method
        ...
        bin/launch.py sets PYTHONPATH=ROOT). This method's venv is `.venvs/2_vae`
        (unpadded); the driver looks for `.venvs/02_vae`, does not find it, and
        falls back to the current interpreter -- which is what runs the test."""
