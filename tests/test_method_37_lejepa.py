...
PRETRAIN_ONLY = {"num_workers": 0, "epochs": 1, "batch_size": 4, "lr": 1.0e-3,
              "min_lr": 1.0e-6, "beta1": 0.9, "beta2": 0.95, "eps": 1.0e-8,
              "weight_decay": 0.05, "warmup_epochs": 0, "clip_grad": 0.0}
TRAIN = {**MODEL, **AUG, **LEJEPA, **PRETRAIN_ONLY}
...
class TestTheDeviceIsResolved(Base):
    def trainer(self):
        return load("lejepa_trainer", METHOD / "train_pretrain_lejepa.py")
    ...
    def test_run_resolves_the_device(self):
        import ast
        src = (METHOD / "train_pretrain_lejepa.py").read_text()
...
class TestTheOriginalIsReferencedNotCopied(unittest.TestCase):
    def test_no_distributed_or_tensorboard_machinery_is_used(self):
        import ast
        tree = ast.parse((METHOD / "train_pretrain_lejepa.py").read_text())
        used = set()
...
# recipe. LeJEPA's backbone is ALREADY vit_base_patch16_224 and its optimiser is
# already AdamW + fixed wd + cosine-to-min_lr, so the unified path is the same key
# set plus `save_at_epochs` (milestone checkpoints) under the unified values.
# Selected by an explicit `recipe: unified` key. The smoke reuses the tiny vit
# (the `name` key), testing the recipe/milestone plumbing, not the real ViT-B.
UNIFIED_TRAIN = {"recipe": "unified", **MODEL, **AUG, **LEJEPA, "num_workers": 0,
                 "epochs": 2, "batch_size": 4, "lr": 6.0e-4, "min_lr": 1.0e-6,
                 "beta1": 0.9, "beta2": 0.95, "eps": 1.0e-8, "weight_decay": 0.05,
                 "warmup_epochs": 0, "clip_grad": 0.0, "save_at_epochs": [1, 2]}
...
class TestUnifiedConfigTranslation(Base):
    ...
    def test_the_unified_only_key_is_refused_on_the_native_path(self):
        # save_at_epochs is the one unified-only key (the sets are otherwise a
        # superset), so the native path must refuse it by name.
        with self.assertRaises(adapter.ConfigError) as e:
            adapter.to_run_config(
                self.config(train={"save_at_epochs": [1, 2]}), self.out)
...
BACKBONE_DIM = 768  # vit_base_patch16_224 num_features (the shipped eval config);
#                     the LeJEPA backbone's pooled feature, one vector per image.
...
class TestFeatureProvider(Base):
    ...
    768-d LeJEPA backbone feature (the ViT's pooled num_features output) -- raw,
    ...
    def _make_encoder(self, cfg: dict) -> Path:
        import torch
        models = load("lejepa_models", METHOD / "models" / "__init__.py")
        train = cfg["train"]
        # A full LeJEPAEncoder (backbone.* + projector.*); extract_encoder keeps
        # only the bare backbone, exactly as the adapter's body() writes encoder.pt.
        model = models.build_lejepa(
            model_name=str(train["name"]), img_size=int(train["img_size"]),
            drop_path_rate=float(train["drop_path_rate"]),
            proj_hidden_dim=64, proj_dim=32, proj_layers=2, final_bn=True)
    ...
    def _provider(self):
        return load("lejepa_feature_provider", METHOD / "feature_provider.py")

    @needs_deps
    def test_it_returns_raw_768d_features_one_per_val_image(self):
        prov_path = METHOD / "feature_provider.py"
        if not prov_path.is_file():
            self.skipTest("37_lejepa provider not yet present")
        import numpy as np
        ...
        self.assertEqual(feats.shape[1], BACKBONE_DIM,
                         "LeJEPA ViT pooled feature is 768-d")
