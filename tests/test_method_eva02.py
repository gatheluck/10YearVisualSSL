#!/usr/bin/env python3
"""Specification for eva02 (EVA-02; Fang et al., 2023; arXiv:2303.11331).

A **pure eval-only** port: a single `linear_eval` stage and no pretraining at all,
the timm-sourced sibling of the download-and-probe methods (36_franca / 30_aim).
EVA-02's "Step 3 as-is" comparison freezes the official masked-image-modelling
pretrained backbone (`eva02_base_patch14_224.mim_in22k`, a ViT built with SwiGLU,
sub-LN and 2D RoPE) and fits a linear probe on its pooled features, because the
from-scratch MIM pretraining (IN-22k, many GPU-days) is the excluded step. The
...
smoke builds a **random tiny** EVA-02 (empty ckpt), so nothing is downloaded.
Licence: the EVA-02 weights are MIT (timm's redistribution); nothing is copied
here.
"""
...
needs_deps = unittest.skipUnless(
    HAVE_DEPS, "eva02 needs torch, torchvision and timm")
...
adapter = load("eva02_adapter", METHOD / "adapter" / "__init__.py")

# A frozen backbone small enough to probe on a CPU: a random tiny EVA-02 (ckpt
# empty) -- 2 small blocks at 32px (patch 16 -> a 2x2 token grid), embed_dim 32,
# 2 heads. The shipped config pins the official EVA-02-B/14 (embed 768, 12 blocks)
# via bin/fetch-weights.py.
EVAL_TRAIN = {"name": "eva02_base_patch14_224", "ckpt": "", "img_size": 32,
...
class Base(unittest.TestCase):
    def setUp(self) -> None:
        self.tmp = Path(tempfile.mkdtemp(prefix="eva02-"))
...
class TestItIsAPureEvalOnlyPort(unittest.TestCase):
    ...
    def test_the_method_name_carries_no_number_and_no_step_word(self):
        self.assertEqual(adapter.METHOD, "eva02")
...
class TestTheBackboneRepresentation(Base):
    def evaluator(self):
        return load("eva02_eval", METHOD / "evaluate_linear_eva02.py")
