...
# frozen-backbone probe. C1:videomae is `done`; C2:vjepa2 (order 20) -- V-JEPA 2,
# an eval-only frozen-backbone probe over the official public facebook/vjepa2-vitl-fpc64-256
# weights (MIT) -- is `done`; and C2:vjepa2_ac (order 21) -- V-JEPA 2-AC, the faithful
# submodule+rope eval-only probe that builds the ViT from the pinned facebookresearch/vjepa2
# submodule and runs the real rotary attention over the official vjepa2-ac-vitg.pt (MIT) --
# is now `done` too (ported 2026-09-02, in turn: order 21 sits before the new `next`),
# so `next` advances to C2:vjepa2_1 (order 22), the
